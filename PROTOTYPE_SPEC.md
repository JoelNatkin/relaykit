...
- **Bottom CTA block**: a full-width 60px (`h-15`) brand "Get early access" button that opens the waitlist modal, followed below by a pre-launch supporting paragraph (`text-sm text-text-secondary`, `mt-4` from the CTA). Action first, context second — reorder lands the CTA above the hedging copy so the paragraph supports rather than softens the click. Pre-launch-superseded originals recorded in `docs/PRE_LAUNCH_DEVIATIONS.md`. **Hidden on every disabled elig bucket (🟠/⚫/🔴)** per the elig section's verdict gating — Wave 2 PM review revision Session 121. On 🔴 there's no future state to sign up for; on 🟠/⚫ the inline waitlist in the elig verdict card is the canonical capture and a backup CTA here would produce untagged general-waitlist signups that bypass the `interest_tag` segmentation.

### Configurator Elig section — `/` (home, right column, above tone pills)
**Files:** `marketing-site/components/configurator/elig-section.tsx`, `elig-dropdown.tsx`, `elig-verdict-card.tsx`, `elig-empty-state.tsx`, `elig-per-category-card.tsx`; `marketing-site/lib/configurator/use-elig-state.ts`, `elig-copy.ts`.
**Status:** Stable structural build (Session 121, Waves 1–3, three commits + revision). Built per `/explorations/vertical-constraints.md` §9 and D-422. Examples expander content and per-vertical primer pages remain out of scope per §9.8 (sniff-test-gated, customer-pull-driven).

Customer-facing eligibility surface that answers the §1 two-question model ("can my business use RelayKit?" + "can I send this text?") via three label-less dropdowns, surfaces a verdict card by D-422 bucket, and shapes the messages section below it accordingly. Reads from `/lib/constraints/` (8 verticals, 137 sub-verticals, 12 content rules). Sits in the upper-right of the configurator section, above the message tone pills; on mobile (below `md:`) it sits between the mobile categories summary and the tone pills.

**State + persistence.** State lives in `useEligState()` (`marketing-site/lib/configurator/use-elig-state.ts`), persisted to `localStorage.relaykit_elig` (`version: 1`, debounced ~200ms, version-gated drop, fail-silent). **Lazy-create rule:** no localStorage write fires until the visitor's first interaction — the key does not exist for untouched visitors. Persisted shape: `{ version, multiTenant, verticalSlug, subVerticalSlug, bucket, verdict: { tier, copy }, updatedAt }` — durable handoff to the future onboarding wizard per MASTER_PLAN principle #7 (continuity of intent).

**Verdict derivation.** Pure function in the hook: (1) `multiTenant === "multi"` → ⚫ Not yet (multi-tenant routes here regardless of D2/D3 per §9.3); (2) `verticalSlug + subVerticalSlug` both set → `lookupEligibility`'s bucket mapped to tier (`Clear`→`clear`, `Conditional`→`conditional`, `Not yet, maybe not ever`→`not-yet-maybe-not`, `Not yet`→`not-yet`, `Not our lane`→`not-our-lane`); (3) `verticalSlug` set + vertical has zero `routingTrigger: true` rows → verdict resolves at D2 from the single shared bucket (currently unreachable in data — every vertical has at least one routing-trigger row — but kept for data robustness); (4) else `null` (placeholder / free-use mode). `verdict.copy` is left null at derive time; rendering pulls authored copy from `elig-copy.ts` at render.

**Three dropdowns (§9.2):**

1. **D1 multi-tenant** — placeholder *"My app sends messages..."*; options *"for one business (mine)"* / *"for many businesses (my customers)"*. Selecting `"multi"` immediately routes to ⚫ without requiring D2/D3.
2. **D2 vertical** — placeholder *"What industry does your app serve?"*; options from `VERTICALS` (8 verticals).
3. **D3 sub-vertical (conditional)** — surfaces only when the picked vertical has any `routingTrigger: true` sub-vertical. **Options include ALL sub-verticals of that vertical** (not just `routingTrigger: true` rows) so users can land on `routingTrigger: false` rows like cannabis/firearms. Dynamic placeholder via `eligD3Placeholder(vertical)` — default `"What kind of {vertical-name-lowercased}?"` with hand-edited overrides for 5 awkward vertical names (home-local-services → "home or local services"; retail-hospitality → "retail or hospitality"; creator-community → "creator or community work"; b2b-saas → "B2B SaaS or developer tooling"; civic-public-sector → "civic or public-sector work"). When D3 is required, verdict stays null until it's answered. Default state (all placeholders) = free-use mode; messages section works as the lead magnet it already is.

**Reset affordance.** Inline `×` (XClose icon) appears at the right edge of each dropdown when a value is selected. Click clears that dropdown back to placeholder. Each dropdown independent. Changing D2 also clears D3 (sub only makes sense within a vertical).

**Verdict card under the dropdowns.** Switch component keyed by tier — five rendering branches per §9.3:

- **🟢 Clear** — inline single line `"You're set — pick your messages below."` with `CheckCircle` icon in `text-fg-success-secondary` and line in `text-text-tertiary`. No card surface, no border.
- **🟡 Conditional** — neutral lifted-surface card (`bg-bg-secondary` + `border-border-secondary`) with `CheckCircle` icon, collapsed line `"You're set — we'll flag the rules where they apply."`, and a "More info / Less info" expander revealing 1–3 paragraphs. Expanded copy is sourced from `getConditionalExpanded(subVerticalSlug)` in `elig-copy.ts`: three §9.4 anchored cards (legal-practice-tools / banking-budgeting-apps / healthcare-administrative, each three paragraphs following the channel reality → vivid trio → consequence pattern); generic fallback for the 22 non-anchored 🟡 sub-verticals (two paragraphs mirroring the anchored opener + a generic rules-apply line, no "we shape your messages by default" closer because that promise only holds where we've actually shaped them).
- **🟠 Not yet, maybe not ever** — warning-toned card (`bg-bg-secondary` + `AlertCircle` icon in `text-fg-warning-secondary`, primary line in `text-text-warning-primary`). Primary line: `"This one's harder than we're set up for today. Get notified if it becomes available."` Inline waitlist email form below — POSTs to `/api/early-access` with `interestTag: "vetting:{subVerticalSlug}"` and `ctaSource: "elig-section"`. Success state replaces the form with `"You're on the list — we'll email you when this is ready."`.
- **⚫ Not yet** — same card chrome as 🟠. Generic copy: `"Coming soon. Get notified when it becomes available."` Multi-tenant gets tailored copy: `"Multi-tenant SMS — sending on behalf of customers — isn't part of launch. Get notified when support lands."` Inline waitlist form POSTs with `interestTag: "capacity:{subVerticalSlug}"` (sub-vertical ⚫) or `interestTag: "multi-tenant"` (D1 = multi).
- **🔴 Not our lane** — same card chrome, no waitlist. Five §9.6 anchored lines keyed by sub-vertical slug (cannabis / firearms / vape / adult-content / adult-dating); surveillance gets its primary line here while the secondary `"Try searching for another provider."` lands in the empty state below per §9.6's two-tier carve-out; generic fallback `"We're not able to send messages for {sub-vertical-name}. Try searching for a specialized provider."` for non-anchored 🔴 rows (currently covers debt-collection-collections-tooling, the 7th 🔴 in the data not anchored in §9.6).

**Brand-system note.** §9.3's literal "blue info card" (🟡) and "orange info card" (🟠/⚫/🔴) are expressed via neutral lifted surface + semantic icon/text colors. The post-D-405 monochrome warm-neutral palette has no chromatic accent except the semantic warning/error/success scales — green check icon (success-secondary) for 🟢/🟡, orange alert icon + orange primary text (warning-secondary + warning-primary) for 🟠/⚫/🔴.

**Verdict-driven gating across the configurator section (Wave 2 + revision).** When the verdict tier is in the disabled set (🟠/⚫/🔴):

- **Categories panel** (desktop + mobile) wrapped in `opacity-50` + `pointer-events-none` + outer `cursor-not-allowed`. Existing checked state preserved per D-422 §5.6 — re-selecting an in-scope vertical brings the user back to their prior shape. Toggles are visually disabled but state mutations are no-op via `pointer-events-none`.
- **Tone pills row + Copy + kebab + business name input** hidden — dead UI when no message stream is rendering.
- **Message stream** replaced by `<EligEmptyState>` — fixed `min-h-72` structural placeholder with dashed border + `Inbox01` icon + bucket-keyed line. Sized for future illustration drop-in per §9.8 (illustration TBD, one-vs-three TBD). Surveillance second-tier line surfaces here.
- **Bottom CTA hidden** on every disabled bucket per Wave 2 revision (`5284d35`). On 🔴 there's no future state to sign up for; on 🟠/⚫ the inline waitlist in the verdict card is the canonical capture and a backup CTA would produce untagged general-waitlist signups that bypass the `interest_tag` segmentation.

**Per-category cards (Wave 3, §9.5).** When the verdict is 🟡, render `<EligPerCategoryCard>` under each affected category's header in the message stream (between the heading row and the optional Variables expander). `isCategoryAffected(subVerticalSlug, categoryId)` walks the sub-vertical's `contentRules`; if any rule's `categoriesAffected` (Session 120 schema field) is absent it applies to all eligible categories, if present it gates by category slug. **Verification carve-out:** `VERIFICATION_CATEGORY_ID` (`"verification"`) is hardcoded out — Verification is content-neutral by nature and never receives a per-category card regardless of vertical. Three §9.5 anchored lines keyed by sub-vertical slug (legal/banking/healthcare-admin); generic `GENERIC_PER_CATEGORY` fallback reserved for future data evolution. The card surfaces a collapsed line + a non-functional `Examples [▾]` affordance — expander content deferred per §9.8 (sniff-test-gated, customer-pull-driven). Production-current behavior: only the 3 anchored 🟡 sub-verticals trigger per-category cards (the only sub-verticals with populated `contentRules` today); 22 unanchored 🟡 sub-verticals render the 🟡 verdict card from Wave 2 but no per-category cards.
