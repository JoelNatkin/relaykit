...
- **Message-card title tooltips.** Every corpus message card (read and edit) surfaces a `HelpCircle` info icon beside the title; hovering it shows the message's optional `tooltip` field from the corpus (`Tooltip`, 1.5s delay). Custom-message cards carry no tooltip — there is no corpus source.
