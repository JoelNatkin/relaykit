...
- **Compliance feedback** (below editor, above pills): opt-out check + required-variable check. Error labels pull directly from the same label source as the `+ Variable` dropdown (`getExampleValues(categoryId).label`), so the two can never drift. Formatting: `Needs X` (one), `Needs X and Y` (two), `Needs X, Y, and Z` (three+). 2s debounced reveal. Compliance is suppressed until the user actually types via a `hasUserEdited` flag — first open on an untouched message shows no errors.
- **Fix button** (formerly "Restore"): appears alongside the compliance hints. Restores the last canned pill's clean template. 1.5s simulated async ("Fixing…") then the editor content is re-seeded, compliance clears, and `hasUserEdited` resets to false (so post-Fix state is clean by construction — a subsequent compliance re-run won't flip errors back on).
