...
- **Bodies use `{{double-brace}}` variable syntax.** `marketing-site/lib/message-library/render.ts` provides `interpolateBody` / `extractTokens` / `flattenBody`; `business_name` (plus the `workspace_name` and `community_name` identity tokens collapsed to it per D-413) resolves from the live businessName input, other tokens from each `Variable.example`.
