...
**Default state + toggle cascade (D-409).** Page-load seeds the default category (`DEFAULT_CHECKED_CATEGORY` = "verification") with every message checked; every other category and every other category's messages start unchecked. Toggling any category ON cascades `checked=true` to every message in it; toggling OFF cascades `checked=false`. The "Verification only" preset reset does the same — every Verification message checked, every other category emptied. The prior `DEFAULT_CHECKED_MESSAGES` per-message default list is deleted; seed and preset derive from `DEFAULT_CHECKED_CATEGORY` alone. Tone overrides and visitor custom messages survive the cascade.

**Authored vs. "Coming soon".** A category is authored when it has non-empty `messages`, `variables`, and `compliance.rules` (`isAuthored()` in the barrel). At launch **Verification**, **Appointments**, **Order updates**, **Customer support**, **Team alerts**, and **Account events** are authored; the other 3 categories (Community, Waitlist, Marketing) render in a disabled "Coming soon" state. Per D-408, every category is a flat list of messages — no `subs`, `stages`, or `classification` distinguishes them at the corpus or render level.
...
- Categories list, one row per corpus category, in this order: Verification / Appointments / Order updates / Customer support / Team alerts / Community / Waitlist / Account events / Marketing. Marketing moved to last position Session 101 — the prior position 2 placed a "Coming soon" tile between Verification and the next live category, which was visually noisy. **Authored categories (Verification + Appointments + Order updates + Customer support + Team alerts + Account events):** active checkbox; collapsed shows category description; expanded (checked) hides the description and reveals indented per-message checkboxes (D-408 — one row per `Category.messages` entry, uniform across categories). **The 3 unauthored categories:** disabled checkbox, category name + an inline "Coming soon" badge (`ComingSoonBadge` — `bg-bg-secondary` / `text-text-tertiary`), category description always shown, no expander.
