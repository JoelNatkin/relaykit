...
**Status:** Live on `main` (merged Session 146, `--no-ff`), but **`noindex, follow` and excluded from the sitemap** — a reachable near-twin of `/messages/account-events`, still iterating. Not a public/indexed surface yet. A2 (Phase 1C) generalizes it into a dynamic `app/for/[slug]/page.tsx` over the registry and lifts noindex.
...
**Registry — `lib/landing/sub-verticals.ts`.** `WorkflowStep` / `Workflow` / `SubVerticalLanding` interfaces + `SUB_VERTICAL_LANDINGS` + `findSubVerticalLanding` / `subVerticalSlugs`. As of Session 148 it holds **14 entries** — `developer-tools` (9 workflows), `customer-support-saas`, `project-management-saas`, `identity-auth-saas`, `team-chat-saas`, `analytics-bi-saas`, `crm-saas`, `hr-hris-saas`, `applicant-tracking-saas`, `compliance-grc-saas`, `cybersecurity-saas`, `edtech-saas`, `esignature-saas`, `logistics-fleet-saas` — but **only `developer-tools` has a route** (`app/for/developer-tools/`); the other thirteen are registry data with no `/for/{slug}` page until the A2 dynamic route is built. `SubVerticalLanding` also carries an optional `variablesExample?: VariablesExample` field (added Session 146, undefined on every entry today). Workflows are a **display layer over the universal corpus** (D-436; MASTER_PLAN Phase 1C "Workflows are a display layer") — each step points at a corpus message by `corpusId` (`"category-id:message-id"`) or carries `customVariants`, and `variableAliases` give contextual placeholders. CC writes this registry from the research library; PM authors. Never Airtable (D-421 AIRGAP).
