...
7. **Prove** (`id="test"`, `Test()` in `app/page.tsx`) — eyebrow "The test", H2 "Run test messages through real phones.", two paragraphs; the `preview-list-mock.tsx` (Joel / Sarah verified, Mike invited) sits in the left grid column, copy at right. Grid `items-start`.
