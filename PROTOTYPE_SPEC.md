...
**Status:** Stable (corpus rewrite Session 95; severity model + decision API Session 106 / D-414 / D-415)
...
- **Editor body** (Tiptap `MessageEditor`, `marketing-site/lib/editor/`): consumes the corpus `{{double-brace}}` body format; variable tokens render as atomic chips (D-350); the chip preview values resolve through `resolveVariableExample({ businessName, categoryVariables })` so authored Edit-values entries reflect inline as the visitor types (D-414). Enter suppressed (single-line SMS).
- **Compliance row** (right-aligned): one line per issue from `checkCompliance`, color-coded by severity (D-415) — warnings in `text-text-warning-primary` (currently: over-segment length, "Over 160 characters — counts as 2 messages."), blockers in `text-text-error-primary` (currently: non-GSM-7 characters, missing STOP/opt-out on Marketing-shaped categories). Save is disabled while any blocker is present (`!compliance.isCompliant` where `isCompliant` means "no blockers"); warnings alone do not disable Save — they're surfaced for the visitor's awareness but the configurator is the lenient authoring stage. No "Fix" button — clicking a tone pill restores a known-good canned variant.
- **Tone-pill row + "+ Variable" popover**: one pill per tone the message offers (`message.variants`); a dashed "Custom" pill appears once the body is hand-edited. "+ Variable" lists the category's `Variable[]` catalog; each row shows the token name + its preview value (also resolver-aware so authored Edit-values entries appear here); click inserts an atomic chip.
