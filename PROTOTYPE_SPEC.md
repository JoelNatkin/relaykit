...
- "Categories" h3 at top, with a permanent pricing-clarity note below it (`text-xs text-text-tertiary`): "All categories included in $19/mo. Marketing adds $10/mo when you go live."
...
- **Top row**: page-level tone pills (Standard / Friendly / Brief, `rounded-full`) + Copy button (tertiary, text + Copy01 icon). Copy flattens every selected message body to plain text.
- **Personalize input** below the top row: a single "Your business name" text input. (The prior `website` input was dropped — no live corpus message uses a website token.)
- **Category groups** (`space-y-7`): one per checked category, each with a `text-base font-semibold` title.
- **Message read-cards** (corpus messages): rounded-xl border, shadow-xs. Name at top + edit pencil; body below renders via `interpolateBody` with `{{token}}` variables as brand-purple inline spans. One card per checked sub's message.
...
Top-to-bottom:
- **Header row**: message name at left; "Edit" label + pencil icon at right.
