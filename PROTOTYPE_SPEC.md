...
**Authored vs. "Coming soon".** A category is authored when it has non-empty subs/stages **and** variables **and** compliance rules (`isAuthored()` in the barrel). At launch **Verification** and **Account events** are authored; the other 7 categories render in a disabled "Coming soon" state.
