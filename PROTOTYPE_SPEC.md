...
**Registry — `lib/landing/sub-verticals.ts`.** `WorkflowStep` / `Workflow` / `SubVerticalLanding` interfaces + `SUB_VERTICAL_LANDINGS` (one entry today, **developer-tools**, 9 curated workflows) + `findSubVerticalLanding` / `subVerticalSlugs`. Workflows are a **display layer over the universal corpus** (D-436; MASTER_PLAN Phase 1C "Workflows are a display layer") — each step points at a corpus message by `corpusId` or carries `customVariants`, and `variableAliases` give contextual placeholders. CC writes this registry from the research library; PM authors. Never Airtable (D-421 AIRGAP).
