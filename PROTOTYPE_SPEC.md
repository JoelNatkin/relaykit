...
**Brand system (D-405, branch `feat/warm-monochrome-brand`).** The marketing site runs one monochromatic warm-neutral palette — Untitled UI's "Gray Warm" scale (`--color-brand-*` in `marketing-site/app/globals.css`; `--color-gray-*` mirrors it), with `brand-950` customized to `#13120E` for a deeper dark-mode page. There is no chromatic accent: value (lightness), not hue, carries surfaces, text, borders, and lifted elements. Anchor tokens — **dark:** page `bg-primary` = `brand-950`, panels `bg-secondary` = `brand-900`, cards/lifted `bg-tertiary` = `brand-800`; **light:** page `bg-primary` = `brand-25`, panels `brand-50`, cards `brand-100`. Lifted/actionable elements — the "Get early access" CTA, selected tone pills, checked checkboxes — use `brand-800` in both modes with white text, hover one step. Variable highlights in message previews use `text-variable` (a quiet neutral step). Semantic colors (error / warning / success) are untouched. The RelayKit wordmark is neutral-scale text. Supersedes the prior purple accent system.
...
1. **Header** — H2 "Configure your messages" (`text-2xl`), subhead "All messages included — yours to copy and use with any provider today." (pre-launch copy — see `docs/PRE_LAUNCH_DEVIATIONS.md`). Section's outer wrapper uses `pt-[100px] pb-16 sm:pb-20` per the home-page uniform 100px inter-section rule.
2. **Two-panel grid** — `md:grid-cols-[3fr_7fr]`, `gap-8`. Left = Categories card (rounded-xl border, `md:min-w-60`); right = Messages column (borderless, `md:max-w-[540px]`, `flex flex-col`).
...
**Authored vs. "Coming soon".** A category is authored when it has non-empty subs/stages **and** variables **and** compliance rules (`isAuthored()` in the barrel). At launch only **Verification** is authored; the other 8 categories render in a disabled "Coming soon" state.
...
- **Bottom CTA block**: a pre-launch supporting paragraph + a full-width 60px (`h-15`) brand-purple "Get early access" button that opens the waitlist modal. Pre-launch-superseded originals recorded in `docs/PRE_LAUNCH_DEVIATIONS.md`.
...
- **Message-card title tooltips.** Every corpus message card (read and edit) surfaces a `HelpCircle` info icon beside the title; hovering it shows the message's optional `tooltip` field from the corpus (`Tooltip`, 0.75s delay). Custom-message cards carry no tooltip — there is no corpus source.
...
**PostHog instrumentation.** Six snake_case conversion events fire client-side: `configurator_category_toggled`, `configurator_sub_toggled`, `configurator_message_customized` (once per message per session, on first override), `configurator_custom_message_added`, `configurator_copy_clicked`, and `early_access_clicked` (fired inside the waitlist context's `openModal`, carrying the configurator snapshot + CTA source). No PII in any payload.
