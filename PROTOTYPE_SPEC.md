...
**Authored vs. "Coming soon".** A category is authored when it has non-empty `messages`, `variables`, and `compliance.rules` (`isAuthored()` in the barrel). At launch **Verification**, **Account events**, **Order updates**, and **Team alerts** are authored; the other 5 categories render in a disabled "Coming soon" state. Per D-408, every category is a flat list of messages — no `subs`, `stages`, or `classification` distinguishes them at the corpus or render level.
