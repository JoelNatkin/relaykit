...
1. **Header** — H2 "Configure your messages" (`text-2xl`), subhead "All messages included — yours to copy and use with any provider today." (pre-launch copy — see `docs/PRE_LAUNCH_DEVIATIONS.md`). 60px gap from the bottom of the hero subhead. Section's outer wrapper uses `pt-[100px] pb-16 sm:pb-20` (was `pt-15 pb-16 sm:pb-20`) per the home-page uniform 100px inter-section rule.
...
- "Categories" h3 at top, with a permanent pricing-clarity note directly below it (`text-xs text-text-tertiary`): "All categories included in $19/mo. Marketing adds $10/mo when you go live."
...
- Categories list, one row per vertical (Verification / Appointments / Order updates / Customer support / Marketing / Team alerts / Community / Waitlist). Each row: checkbox + title + description (always visible, drawn verbatim from the prototype's onboarding `categories.ts`). **All categories including Verification are toggleable (D-377)** — no "Included" pill, no `disabled` props on the checkbox or button. The Marketing row's title carries an inline permanent "(+$10/mo)" marker (`text-xs text-text-tertiary`); Marketing's row, when selected, also shows a "Requires EIN. Adds a few days to registration." note. Rows use `border-b border-border-secondary py-5`. **When zero categories are selected**, the right-pane message-preview area shows a centered placeholder with a dashed border: "Pick a category to see message previews." (`text-sm text-text-tertiary`).
