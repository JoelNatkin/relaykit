...
### Home page (`/`) — v10 marketing home (Session 127; D-427/D-428; reworked Sessions 129–131)
...
**Eyebrow system ("The ___").** Every section opens with an `<Eyebrow>` (`components/home/section-ui.tsx`; gold dot) whose label is a uniform **"The {noun}"** — The problem / The process / The messages / The paperwork / The build / The test / The price / The start. The **hero carries no eyebrow**. (This replaces the prior mixed eyebrow phrasings — "Why RelayKit", "How it works", "What we handle", etc.)
...
2. **StatusBand** (`StatusBand()` in `app/page.tsx`, no `id`) — a slim centered band (`py-8`) between Hero and Recognition: "Our free message templates are live." over a tertiary "Sending arrives Summer 2026."
3. **Recognition** (`id="why"`, `Recognition()` in `app/page.tsx`) — eyebrow "The problem", H2 "The rules show up after you start building.", subhead. Then a **single full-width requirements accordion** (one `bg-surface-card` card holding six native `<details name="requirements">` rows — one open at a time): Registration / Carrier review / Consent requirements / Build a compliance website / STOP and HELP handling / Message restrictions. Each summary row is an error-red dot + title + a `ChevronDown` that rotates on open; the open body (capped `max-w-[460px]` for line length) carries the requirement explanation plus a closing "RelayKit …" relay line. (Replaces the prior two-card "You expected days" / "You got weeks" model.)
4. **How it works** (`id="how"`, `how-it-works.tsx`) — eyebrow "The process", H2 "What it takes to go live.", subhead "No telecom expertise required. We handle the carrier side.", then a **three-step** `<StepStrip>` (open variant): 01 · PLAN **Choose your messages** (gold link "Browse the messages →" → `/messages`) → 02 · BUILD **Build and test** → 03 · LAUNCH **Go live**, each carrying a cumulative gold progress bar (`w-1/3` → `w-2/3` → `w-full`). (Was a four-step grid.)
5. **Configurator** (`id="configurator"`) — eyebrow "The messages", H2 "Messages for every job.", subhead, a gold "Open Messages →" link (→ `/messages`), then the **real `<ConfiguratorSection/>` embedded in a clipped `h-[640px] overflow-hidden` window** (full-bleed via `mx-[calc(50%-50vw)]`), offset up `-mt-[56px]` (trims the configurator's deterministic 80px `pt-20` so the window opens cleanly on its own "Write your messages" header) with a bottom fade gradient. One live, fully-interactive instance, sharing `relaykit_configurator` / `relaykit_elig` localStorage with `/messages` (continuity of intent, MASTER_PLAN #7; the two routes never co-mount). Below it: a trust line ("Copy the templates and use them anywhere today … Twilio, Sinch, Telnyx, custom infrastructure …"). (The former `VariablesCallout` before→after block that sat here was removed — its role is now the standalone "The variables" section below, D-434.)
6. **The variables** (`id="variables"`, `variables-section.tsx`, **D-434**) — eyebrow "The variables", H2 "Make the messages yours, without breaking them.", bridge ("Preview every message, edit the wording, or add your own fields — the parts that keep you compliant stay locked."), then a 1-col→2-col (`md:`) grid of two **presentational-static** cards illustrating the authoring experience (no live editor, no state, server component): **"Preview with your data"** (body "Enter a value once and it shows in every message."; an active field with a two-part label row — human "Provider name preview" left, mono slug `provider_name` right — a lighter `border-fg-quaternary` active border + a static gold caret, over a Confirmation message-editor mock with the `Jordan Lee` value gold-tinted, dimmed tone-pills/Insert-variable/Save) and **"Add a field"** (the same editor mock with `summitfitness.com/reschedule` gold-tinted + an **open** "Insert variable" menu — right-anchored `w-[calc(100%-16px)]`, `max-h-[134px]` overflow-hidden, seven variable rows with the `reschedule_link` row selected under a decorative mouse cursor, plus an always-on fake scrollbar). Geometry ported 1:1 from the dev-tools landing mockup; replaced the prior `VariablesCallout` / "See exactly what customers will receive." block. Renders between `#configurator` and `#rules`.
7. **Paperwork** (`id="rules"`, `paperwork.tsx`) — eyebrow "The paperwork", H2 "You build the feature. We handle the bureaucracy.", three cards (Registration handled / Messages compliant / Opt-ins & opt-outs covered) with gold-tint (`bg-bg-gold/15`) featured-icon backgrounds.
8. **Build** (`id="build"`, `ai-section.tsx`) — eyebrow "The build", H2 "Give your AI tool a build spec, not a pile of docs.", body, an AI-tool wordmark row (Claude Code, Cursor, Windsurf, GitHub Copilot, Cline, Codex — dark/light asset swap via `negSrc` or a `brightness-0 dark:invert` fallback), and "Slots into ShipFast, Supastarter, MakerKit, and Vercel + Supabase." Right column: a `CodeCard` carrying the **real SDK signature** (`new RelayKit()` / `appointments.sendConfirmation(...)`), gold identifier highlights, caption "That's the send." Grid is `items-start`; both columns `min-w-0` so the code block scrolls internally on mobile.
9. **Prove** (`id="test"`, `Test()` in `app/page.tsx`) — eyebrow "The test", H2 "Run test messages through real phones.", two paragraphs; the `preview-list-mock.tsx` (Joel / Sarah verified, Mike invited) sits in the left grid column, copy at right. Grid `items-start`.
10. **Pricing** (`id="pricing"`, `pricing.tsx`) — eyebrow "The price", H2 "Simple pricing." Left: a single card with two stacked stages, each labelled by a **gold mono "Stage 1" / "Stage 2"** label (D-427) — "Build for free" (set up, add the code, test with real phones, no credit card) / "Go live for $49 + $19/mo" (we file your registration; approval takes a few days; 500 messages included per month, then $8 per additional 500; full refund if not approved). Right: "What $19/mo includes." — five gold-tick rows (500 messages a month, your own sending number, delivery/opt-outs/quiet hours, a hosted compliance site, carrier rule-change tracking) + fine print (marketing +$10/mo, volume pricing above 5,000, US & Canada at launch, no HIPAA / healthcare / enterprise). Copy is canon-clean against PRICING_MODEL and uses "a few days" (D-215/D-376) — no day count.
11. **Final CTA** (`id="join"`, `final-cta.tsx`) — eyebrow "The start", H2 "The messages are ready now. **Sending arrives Summer 2026.**" (second sentence tertiary-toned; the `<br>` is `hidden sm:block`), body with an inline gold "start with the messages today" link (→ `/messages`), then a `BottomEmailCapture` (`ctaSource="home-final"`) that posts to `/api/early-access`. (Simplified Session 130 — the prior "Open the configurator →" `PrimaryCta` + "Or get an email…" caption were dropped in favor of the inline gold link.)
