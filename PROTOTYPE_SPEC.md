...
### Home page (`/`) — v10 marketing home (Session 127; D-427/D-428)

The home is a **marketing page**, not the configurator tool. It was rebuilt from the v10 design and is composed from one component per section under `marketing-site/components/home/*`; `app/page.tsx` is a server component and the section components bring their own `"use client"` where needed. The free message configurator — the standalone authoring tool — lives at its own route, **`/messages`** (D-428; see "Configurator tool page — `/messages`" below); the home embeds the *same real* `<ConfiguratorSection/>` in a clipped window rather than forking it. **Dark mode is the site-wide default** (Session 127). A single **metallic-gold accent** (`#E0B010`, D-427) sits over the D-405 monochrome base — primary CTAs, eyebrow dots, how-it-works/configurator progress bars + the "Open the configurator" link, the "RelayKit handles them." resolve line, paperwork featured-icon tints, the code-snippet identifier highlight, pricing ticks + Stage labels, and the "Invited" status dot; green/red carry status/friction *meaning* per D-405. All gold flows through tokens (`--color-gold` etc.); no raw hex outside `globals.css`.

Each section is a full-bleed `<section>` constrained to `max-w-5xl px-6`, separated by `border-t border-border-secondary`, padded `py-20 sm:py-28`, and opens with an `<Eyebrow>` (`components/home/section-ui.tsx`; gold dot). Sections in order:

1. **Hero** (`hero.tsx`) — eyebrow "Shipping Summer 2026", H1 "The easiest way to add text messaging to your app.", a rotating category line (`category-rotor.tsx` — cycles the nine corpus `CATEGORIES` names, monochrome, `aria-live="polite"`), a subhead, a gold `PrimaryCta` "See the messages →" (→ `/messages`) + a `GhostCta` "Join the list" (→ `#join`), and a trust line ("Free to build & test · No credit card · US & Canada"). Right column: a `PhoneMock` SMS conversation with a fixed-size neutral white-alpha **depth glow** behind it (`HERO_GLOW_STYLE` — a phone-scoped radial in a 680×600 box centered on the phone; a dark-mode cue, effectively invisible on the light theme) plus a masked dot-grid texture layer (`DOT_GRID_STYLE`). The phone screen is scoped `dark` so it stays a dark SMS app in both page themes.
2. **Why RelayKit** (`id="why"`, `Recognition()` in `app/page.tsx`) — eyebrow "Why RelayKit", H2 "The feature is small. The paperwork isn't." Two cards (both `border-border-secondary`): "What you expected" (3 items, success-green `→` markers) vs "What you discovered" (6 friction items, error-red dots). Closing line "Most developers discover the second column after they start building. **RelayKit handles them.**" — resolve line in gold.
3. **How it works** (`id="how"`, `how-it-works.tsx`) — eyebrow "How it works", H2 "From idea to live SMS, in four steps.", a four-step grid (01 PLAN Choose your messages → 02 BUILD Hand off the spec → 03 PROVE Test with real phones → 04 LAUNCH Go live), each step carrying a cumulative gold progress bar (`w-1/4` → `w-full`). Step 01 links to `/messages`.
4. **Configurator** (`id="configurator"`) — eyebrow "The configurator · live today", H2 "Start with a complete message plan.", a gold "Open the configurator →" link (→ `/messages`), then the **real `<ConfiguratorSection/>` embedded in a clipped `h-[640px] overflow-hidden` window**, offset up `-mt-[64px]` (trims the configurator's deterministic 80px `pt-20` so the window opens cleanly on its own "Write your messages" header) with a bottom fade gradient. One live, fully-interactive instance, sharing `relaykit_configurator` / `relaykit_elig` localStorage with `/messages` (continuity of intent, MASTER_PLAN #7; the two routes never co-mount). Below it: a trust line ("Copy the templates and use them anywhere today … Twilio, Sinch, Telnyx, custom infrastructure …") and a **VariablesCallout** (`variables-callout.tsx` — three real corpus messages shown as raw `{{token}}` template → resolved preview, side by side).
5. **What we handle** (`id="rules"`, `paperwork.tsx`) — eyebrow "What we handle", H2 "You build the feature. We handle the bureaucracy.", three cards (Registration handled / Messages compliant / Opt-outs enforced) with gold-tint (`bg-bg-gold/15`) featured-icon backgrounds.
6. **Build** (`id="build"`, `ai-section.tsx`) — eyebrow "Build" (full-width above the grid), H2 "Give your AI tool a build spec, not a pile of docs.", body, an AI-tool wordmark row (Claude Code, Cursor, Windsurf, GitHub Copilot, Cline, Codex — dark/light asset swap via `negSrc` or a `brightness-0 dark:invert` fallback), and "Slots into ShipFast, Supastarter, MakerKit, and Vercel + Supabase." Right column: a `CodeCard` on the theme-invariant `bg-bg-code-surface` carrying the **real SDK signature** (`new RelayKit()` / `appointments.sendConfirmation(phone, { date, time })`), gold identifier highlights, caption "That's the send." Grid is `items-start`; both columns `min-w-0` so the code block scrolls internally on mobile.
7. **Prove** (`id="test"`, `Test()` in `app/page.tsx`) — eyebrow "Prove" (full-width above the grid), H2 "Run test messages through real phones.", two paragraphs; right column the `preview-list-mock.tsx` (Joel / Sarah verified, Mike invited). Grid `items-start`.
8. **Pricing** (`id="pricing"`, `pricing.tsx`) — eyebrow "Pricing", H2 "Simple pricing." Left: a single card with two stacked stages, each labelled by a **gold mono "Stage 1" / "Stage 2"** label (D-427) — "Build for free" (set up, add the code, test with real phones, no credit card) / "Go live for $49 + $19/mo" (we file your registration; approval takes a few days; 500 messages included per month, then $8 per additional 500; full refund if not approved). Right: "What $19/mo includes." — five gold-tick rows (500 messages/mo, your own sending number, delivery/opt-outs/quiet hours, a hosted compliance site, carrier rule-change tracking) + fine print (marketing +$10/mo, volume pricing above 5,000, US & Canada at launch, no HIPAA / healthcare / enterprise). Copy is canon-clean against PRICING_MODEL and uses "a few days" (D-215/D-376) — no day count.
9. **Final CTA** (`id="join"`, `final-cta.tsx`) — eyebrow "Start now", H2 "The messages are ready now. Sending arrives Summer 2026." (the `<br>` is `hidden sm:block` so the line wraps naturally on mobile, with an explicit trailing space so the two sentences never touch), body, a gold `PrimaryCta` "Open the configurator →" (→ `/messages`), then "Or get an email when sending launches:" over a `BottomEmailCapture` that posts to `/api/early-access`.

### Configurator tool page — `/messages` (D-428)

`marketing-site/app/messages/page.tsx` (server component) is the free standalone configurator — a product in its own right (MD-21). A thin demand-voice header (H1 "Text messages for your app, ready to copy.", subhead), then the **quick-start orientation strip**, then the real `<ConfiguratorSection/>`. Outer wrapper carries `pb-20 sm:pb-28` so the configurator clears the footer.

**Quick-start strip** (`components/messages-quickstart.tsx`, `"use client"`, **D-429**) — a page-level, dismissible four-step orientation strip shown at rest above the configurator: eyebrow "Quick start" + H2 "Build your message plan in four steps." (`text-2xl`, an orientation strip, not a hero) + a four-step grid reusing How-it-works' exact visual language (cumulative gold progress bar) — 01 Details (Add your business) → 02 Select (Pick your messages) → 03 Personalize (Add your data) → 04 Edit (Make it yours). Bordered container (`border-border-secondary`, `rounded-2xl`, `p-7`), transparent interior (no fill, matching How-it-works against the dark page). An X (top-right, `aria-label="Dismiss"`, from `@untitledui/icons`) hides it and persists to `localStorage.relaykit_quickstart_dismissed`; **hydration-safe** — visibility starts `null` (renders nothing on the server and the first client paint), then resolves from localStorage in a `useEffect`. **Page-level, deliberately NOT inside `ConfiguratorSection`** — so it never leaks into the home's embedded peek.

### Top nav — marketing-site (`top-nav.tsx`)

`"use client"`; fixed bar, RelayKit wordmark (left, → `/`). Right group at `sm`+: the **theme toggle** (left of the links) then the primary links **Messages / Pricing / Blog**. Below `sm`, those hide and a right-aligned **hamburger** (`Menu01` / `X`, 44px tap target, `aria-expanded`) opens a full-screen **mobile menu** — a `role="dialog"` portal to `<body>` mirroring the configurator modal idioms (ESC-to-close, body-scroll lock, sticky header with wordmark + X, body-scrolls layout) with focus moved into the panel on open and restored to the hamburger on close. The menu stacks the same links (each closes the menu on tap) plus a theme-toggle row using the same `useTheme` `toggle`/`theme`. Legal/Resources stay in the footer.

**Dark mode (D-378, branch `feat/dark-mode`).** The marketing site supports light/dark mode via vanilla React + CSS — no `next-themes` dependency. Mechanism: `.dark` class on `<html>`, theme persisted to `localStorage` key `relaykit-theme`, **dark as the site-wide default** (Session 127; the toggle + stored preference override), FOUC prevented by an inline pre-hydration script in `app/layout.tsx` paired with `suppressHydrationWarning`. The `useTheme` hook (`marketing-site/lib/use-theme.ts`) returns `{ theme, toggle }` with a null-during-mount state so the toggle button can render a non-breaking placeholder rather than guess server-side. Top-nav exposes the toggle (label "Light mode" / "Dark mode" depending on current state). The body is painted `bg-bg-primary` so sections without their own background pick up the theme (UA canvas-dark would otherwise diverge from `bg-primary` by ~8 RGB units). New semantic token `bg-bg-code-surface` is used by the Section 3 code block. Brand-color tokens are governed by the **Brand system** note below (D-405), which supersedes D-378's original purple-scale dark shift. Dark-mode iteration commits: `1164c8b` token block in globals.css; `62cf4ee` inline FOUC script + `suppressHydrationWarning`; `c10d549` useTheme hook + lint coverage for `lib/`; `5baa47e` dark-mode toggle in TopNav; `c844609` body painted `bg-bg-primary`; `98641b2` `text-white → text-text-white` on configurator + edit-card brand-solid CTAs; `37e28f5` wordmark `_neg` variants; `82c126c` `bg-bg-code-surface` token + home `<pre>` swap.
...
1. **Header** — H2 "Write your messages" (`text-2xl`, `mb-4` to the body; the former "Templates configured for your app." subtitle was removed Session 128). Section's outer wrapper uses `bg-bg-primary pt-20`; on the home the embedded window trims this via `-mt-[64px]` so it opens cleanly on the header (no bottom padding).
