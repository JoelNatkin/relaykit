...
**Hero rotor note — intentional, do NOT "fix" to a CATEGORIES source.** `category-rotor.tsx` animates a **hardcoded one-pass `FRAMES` list** ("Appointments." → "Order updates." → "Account events." → "Every text your app sends.") and **stops on the final frame — no loop**. It is deliberately **not** sourced from the corpus `CATEGORIES`: the resting line ("Every text your app sends.") is a summary, not a category, so it cannot be derived from `CATEGORIES.map`. A future session should not "consolidate" this back to a CATEGORIES-driven cycle — the fixed list and the stop-on-rest behavior are the design. First frame animates in via a `requestAnimationFrame` reveal (so frame 0 fades in like the rest); `INTERVAL_MS 1700` / `FADE_MS 280`.
