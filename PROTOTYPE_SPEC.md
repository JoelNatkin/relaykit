...
4. **The messages** (`id="configurator"`, `messages-section.tsx`, **D-435**) — eyebrow "The messages", H2 "Messages for every job.", bridge "From login codes to event invites, every message your app sends is ready to use." A lightweight **category-pills message browser** (client component) — **replaced the prior clipped `<ConfiguratorSection/>` embed** (D-435 partially supersedes D-428; the old peek + its "Copy the templates… Twilio, Sinch, Telnyx…" trust paragraph are gone from the home). **Category selector:** 9 categories, single-select — **desktop (`md`+): two pill rows (4 + 5)** in order Account events · Order updates · Appointments · Verification / Customer support · Team alerts · Waitlist · Community · Marketing; **mobile (below `md`): a single horizontal-scrolling pill row** (`flex-nowrap overflow-x-auto`, scrollbar hidden, slightly larger pills `px-4 py-2.5 text-sm`) — not a native select. Default = Account events; selected pill = gold (`bg-bg-gold text-text-on-gold`), others muted. **Controls row** (breakpoint `md`): a business-name input (placeholder "Your business name") carrying a **clear (X) button** at its right edge whenever it's non-empty (clears via `setBusinessName("")`, `pr-10` keeps text off it) + the tone control. **Desktop (`md`+):** input at `md:w-80` + the three tone **pills** Standard/Friendly/Brief; selected tone = **monochromatic neutral** (`bg-border-primary text-text-primary`, NOT gold — gold is reserved for category selection). **Mobile (below `md`):** one row — name input (`grow-[2]`) beside a tone **`<select>`** (`grow`; same Standard/Friendly/Brief, same `tone` state via `setTone`), `gap-3`. Stack: category → 32px → controls → 16px → cards. **Cards:** 2-col grid (`bg-surface-card`) of the selected category's real corpus messages in the active tone, rendered via `interpolateBody` — gold pip + `Message.name` title + body with **every `{{token}}` value bolded** (`font-medium text-text-primary`: business name AND all others); literal copy + the "Reply STOP to opt out." tail stay plain; the business name shows **"Acme"** until the visitor types (presentation only, never written to the store). **Fixed-height dot-paginated carousel:** 6 cards/page (2 cols × 3 rows, `h-[394px]` + `overflow-hidden` at `md:`; stacked auto-height below `md`); centered page dots shown **only when >1 page** (click + swipe to advance), **no bottom gradient**. **"Open Messages →"** gold link bottom-right → `/messages`. Business name binds to the shared `useConfiguratorState` store (`state.businessName` / `setBusinessName`) so the home and `/messages` stay in sync (continuity of intent, MASTER_PLAN #7); tone is local `useState`, not persisted. `/messages` + `ConfiguratorSection` are untouched.
   ...
   **Reusable props (Session 145, Phase 1C).** `MessagesSection` accepts two optional props so it can nest under a shared heading on landing surfaces without duplicating chrome: **`chromeless`** renders only the category pills + cards — no `<section>`/`id="configurator"`, no heading block, no controls row (the parent owns heading + business-name + tone); and a **controlled `tone`** prop (`activeTone = toneProp ?? internalTone`) lets the parent drive the active tone. Both default off/undefined, so standalone/home behavior is byte-unchanged. Consumed by the `/for/developer-tools` MessagesWorkflowsSection (below) — PM-approved as the clean alternative to a second `id="configurator"` + a duplicate heading/controls.
5. **The variables** (`id="variables"`, `variables-section.tsx`, **D-434**) — eyebrow "The variables", H2 "Make the messages yours, without breaking them.", bridge (two sentences with a desktop-only line break between them — "Preview every message, edit the wording, or add your own fields." / "The parts that keep you compliant stay locked."), then a 1-col→2-col (`md:`, 32px gutter) grid of two **presentational-static** cards illustrating the authoring experience (no live editor, no state, server component): **"Preview with your data"** (body "Enter a value once and it shows in every message."; a label-less active field showing `Jordan Lee` + a static gold insertion caret (input only — not in the message previews) with a lighter `border-fg-quaternary` active border, over a Confirmation message-editor mock with the `Jordan Lee` value gold-tinted, a right-aligned dimmed Insert-variable + dimmed Save; **no tone pills**) and **"Customize any message"** (the same editor mock with `summitfitness.com/reschedule` gold-tinted + a right-aligned **open** "Insert variable" menu — right-anchored `w-[calc(100%-16px)]`, `overflow-hidden`, **four** variable rows (`workspace_name` / `provider_name` / `reschedule_link` / `appointment_time`) with the `reschedule_link` row selected under a decorative mouse cursor; **no scrollbar**, value text truncates with ellipsis on narrow widths). Card body copy clears `mb-7` to the field/editor below; each card container is `p-5 pb-7`; gold-tint highlight is `bg-bg-gold/20` with balanced vertical padding (`pt-px pb-[3px]`). Geometry ported from the dev-tools landing mockup (then simplified — field label row, scrollbar, and rows 5–7 dropped; the caret survives on the Card-1 input only). Replaced the prior `VariablesCallout` / "See exactly what customers will receive." block. Renders between `#configurator` and `#build`.
...
### Sub-vertical landing page — `/for/developer-tools` (D-436; Workflows layer Session 145, Phase 1C)

**Files:** `marketing-site/app/for/developer-tools/{page.tsx, sections.tsx, messages-workflows-section.tsx}` + `marketing-site/components/home/workflows-section.tsx` + the registry `marketing-site/lib/landing/sub-verticals.ts`.
**Status:** Live **and indexable** on `main` (`robots: index, follow`; in the sitemap) as of Session 149 (Phase 1C A2). It remains a **bespoke static page** — kept separate from the dynamic `/for/[slug]` route (next section), which serves the other 16 entries; dev-tools is filtered out of `generateStaticParams`, and Next resolves the static segment ahead of `[slug]`. Its hand-authored copy (the "BuildKit 2am" Moment, the "Q&A: Account event messages" heading, the B2B `Farm`) is not modelled by the registry, so it stays static rather than collapsing onto the generic route.

The page follows the D-436 sub-vertical recipe (shared home chrome wrapped around authored hero/moment/details + sub-fed data). Its distinguishing piece is the **messages block**, which leads with curated *workflows* instead of the generic category browser.

**MessagesWorkflowsSection** (`app/for/developer-tools/messages-workflows-section.tsx`, client). Owns one heading + a **Workflows / All messages** segmented toggle (Workflows default) + the shared business-name + tone controls. Resolves its data from `findSubVerticalLanding("developer-tools")`. The selected toggle pill is **gold** (`bg-bg-gold text-text-on-gold`, matching the category pills). Two views:
- **Workflows** → `<WorkflowsSection workflows businessName tone />`.
- **All messages** → `<MessagesSection chromeless tone defaultCategory />` — the home browser nested without its own heading/controls/`id` (via the `chromeless` + controlled-`tone` props documented under "The messages", above).

Business name is the shared `useConfiguratorState` store (drives both views automatically); tone is local `useState` and passed to both. This replaced the standalone `<MessagesSection>` + `<VariablesSection>` pair the page previously rendered.

**WorkflowsSection** (`components/home/workflows-section.tsx`, client). Renders a sub-vertical's curated workflows as **collapsible stepped cards** in a responsive grid (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-3`, `gap-3.5`), **sorted by step count desc** (copies the prop first, never mutates). Each card (`rounded-xl border border-border-secondary bg-surface-card`) is **collapsed by default** (header + step names only) and expands **independently** (per-card `useState<Set<string>>`) to reveal each step's message body; a `ChevronDown` rotates 180°. The per-step left rail is a gold dot (`size-[7px]`, raw `#c9a84c` — not yet tokenized) over a vertical connector (`w-px flex-1 bg-border-primary`, `mt-[5px]` so it sits centered between dots with equal top/bottom gap). Each step's body resolves from the **universal corpus** (`corpusId = "category-id:message-id"`) or the step's own `customVariants`, then substitutes the shared business name, the step's `variableAliases`, and a module-level **`FALLBACKS`** table (display values for ~23 corpus tokens, so an unresolved `{{token}}` never renders literally); injected values are bolded via a longest-first split. Presentational — the parent owns the controls.

**Registry — `lib/landing/sub-verticals.ts`.** `WorkflowStep` / `Workflow` / `SubVerticalLanding` interfaces + `SUB_VERTICAL_LANDINGS` + `findSubVerticalLanding` / `subVerticalSlugs`. As of Session 149 it holds **17 entries** — `developer-tools` (9 workflows, the bespoke static page), plus `customer-support-saas`, `project-management-saas`, `identity-auth-saas`, `team-chat-saas`, `analytics-bi-saas`, `crm-saas`, `hr-hris-saas`, `applicant-tracking-saas`, `compliance-grc-saas`, `cybersecurity-saas`, `edtech-saas`, `esignature-saas`, `logistics-fleet-saas`, `marketing-automation-saas`, `survey-feedback-saas`, `childcare-saas`. **All 17 now render** — `developer-tools` from its static page, the other 16 from the dynamic `/for/[slug]` route (next section). `SubVerticalLanding` also carries an optional `variablesExample?: VariablesExample` field (added Session 146, undefined on every entry today). Workflows are a **display layer over the universal corpus** (D-436; MASTER_PLAN Phase 1C "Workflows are a display layer") — each step points at a corpus message by `corpusId` (`"category-id:message-id"`) or carries `customVariants`, and `variableAliases` give contextual placeholders. CC writes this registry from the research library; PM authors. Never Airtable (D-421 AIRGAP).

**Known carry-forwards.** WorkflowsSection step dot is still raw `#c9a84c` (tokenize to `bg-bg-gold` if desired); `FALLBACKS` display values are CC-chosen defaults (PM may review the copy); `DEVTOOLS_VARIABLES_EXAMPLE` is now an unused export in `sections.tsx` (VariablesSection no longer rendered here).

---

### Sub-vertical landing pages — dynamic `/for/[slug]` (D-436/D-437; Phase 1C A2, Session 149)

**Files:** `marketing-site/app/for/[slug]/page.tsx` (dynamic route) + four generic, data-driven sections in `marketing-site/components/landing/` — `sub-vertical-hero.tsx`, `sub-vertical-moment.tsx`, `sub-vertical-details.tsx`, `sub-vertical-messages-workflows-section.tsx`.
**Status:** Live + indexable on `main` (merged Session 149, `--no-ff` `5c0fd13`). Serves **16** of the 17 registry entries; `/for/developer-tools` keeps its own bespoke static page (above) and is excluded from `generateStaticParams` (Next resolves the static segment ahead of `[slug]`). No new D-number — implements D-436 (sub-vertical recipe) + D-437 (page-type pattern).

The route reuses the **exact D-436 composition** of the dev-tools page, in the same section order, but every per-sub section is **data-driven from the registry `SubVerticalLanding` entry** instead of hardcoded:
- **`generateStaticParams`** → `subVerticalSlugs().filter((s) => s !== "developer-tools")`. **`dynamicParams = false`** (matches `/messages/[category]`) — only known slugs render; any other `/for/*` → `notFound()` (404). All 16 prerender static (+ dev-tools static = 17 total `/for/*` pages).
- **`generateMetadata`** (async `params: Promise`) → per-entry `metaTitle` / `metaDescription`, **self-canonical** `/for/{urlSlug}` (never → `/`), `robots: { index: true, follow: true }`.
- **Composition** (identical order to dev-tools): `SubVerticalHero` · `StatusBand` · `SubVerticalMoment` · `SubVerticalMessagesWorkflowsSection` · `SubVerticalDetails` · `Paperwork` · `PaperworkFork` · `AiSection` · `Prove` · `HowItWorks` · `Pricing` · `NumbersSection` · `Recognition` · `FinalCta` · `Farm`.

**Generic sections** (`components/landing/sub-vertical-*.tsx`) are class-for-class copies of the dev-tools `sections.tsx` equivalents, reading `entry.*`: **Hero** (`heroEyebrow` / `h1` / `heroBody` / `heroExamples` → `HeroNotificationMock`; static CTAs + trust line); **Moment** (static eyebrow "The moment" + h2 "A text can change the outcome."; `moment.body` / `exampleSms` / `exampleReply`); **Details** (static eyebrow "The details" + h2 **"Common questions"** — replaces the dev-tools "Q&A: Account event messages"; maps `entry.qa`); and the **client** `SubVerticalMessagesWorkflowsSection` (prop `entry`; h2 **"{entry.name} messages. And all of the others."**; the same Workflows / All-messages segmented toggle + business-name + tone controls as the dev-tools wrapper, which stays **untouched**). `id="configurator"` is preserved on the messages section so the hero `#configurator` CTA anchors.

**Shared `PaperworkFork`.** The duplicated dev-tools-local `PaperworkFork` was removed; both the dev-tools page and the dynamic route now import the single `components/landing/paperwork-fork.tsx` (also used by `/messages/[category]`).

**Known carry-forwards.** `Farm` is imported as-is from `app/for/developer-tools/sections` (hardcoded B2B directory; links → `/messages` placeholders) — per-vertical Farm content + real pain-point targets are later Phase 1C work, and the cross-route import is a noted smell (relocating `Farm` to `components/landing/` is optional cleanup).

**Sitemap.** `app/sitemap.ts` now derives `SUB_VERTICAL_ROUTES` from `subVerticalSlugs()` (`/for/{slug}`) and includes all 17 alongside `STATIC_ROUTES` + `CATEGORY_ROUTES` (the prior "developer-tools intentionally excluded" note is gone).
