...
**Data source + persistence.** The configurator imports the 9 corpus categories via `@/lib/message-library` and renders from them. All selection/edit state lives in `useConfiguratorState()` (`marketing-site/lib/configurator/use-configurator-state.ts`), persisted across sessions to `localStorage` key `relaykit_configurator` (`version: 1`, debounced ~200ms, version-gated drop, fail-silent; persisted state merges over a fresh corpus seed with id intersection).

**Authored vs. "Coming soon".** A category is authored when it has non-empty subs/stages **and** variables **and** compliance rules (`isAuthored()` in the barrel). At launch **Verification**, **Account events**, and **Order updates** are authored; the other 6 categories render in a disabled "Coming soon" state.
...
- Categories list, one row per corpus category (Verification / Marketing / Appointments / Order updates / Customer support / Team alerts / Community / Waitlist / Account events). **Authored categories (Verification + Account events + Order updates):** active checkbox; collapsed shows category description; expanded (checked) hides the description and reveals indented sub-checkboxes (or stages for workflow categories — same UI shape, different corpus discriminant). **The 6 unauthored categories:** disabled checkbox, category name + an inline "Coming soon" badge (`ComingSoonBadge` — `bg-bg-secondary` / `text-text-tertiary`), category description always shown, no expander.
- **Verification sub-checkboxes** (shown when Verification is checked, indented `pl-7` to align under the title): four subs in order — Signup phone verification / Sensitive-action step-up / Account recovery / Login 2FA. Default: only "Signup phone verification" checked. Each sub row is an entire-row click target (toggles the sub via `role="button"`, Enter/Space keyboard support). When a sub carries a `tooltip`, a small `HelpCircle` `?` icon renders to the right of the label inside an `items-center gap-1.5` inner wrapper — same pattern as the message-card title. Hovering the `?` icon shows the tooltip body centered horizontally over the icon, instantly (no open delay); the icon itself has an invisible 44px tap target via `size-11` wrapper with `-m-[15px]` to preserve the 14px layout footprint; clicking the `?` icon `stopPropagation`s so it doesn't also toggle the row. Hovering the bare label text does **not** trigger the tooltip — the `?` is the only trigger.
- **Account events sub-checkboxes** (shown when Account events is checked, indented `pl-7`): five subs in order — Payment failed / Trial · renewal upcoming / Subscription confirmation / Security event / Account status change. 5 messages, 15 tone variants (3 tones each). Same row-click + `?`-icon tooltip pattern as Verification. The category is authored but **ships unchecked** at launch (only Verification's primary sub is in `DEFAULT_CHECKED_SUBS`); toggling the category on does not auto-check any sub, so the visitor explicitly picks which subs land in the message column. The category introduces `account_link` — a workspace-settings-sourced variable for the developer's own account/billing URL on their own domain (RelayKit does not shorten or host it). The configurator renders a static developer-domain placeholder (`yourapp.com/billing`, budgetChars 19) as the example. Per PM, `account_link` is captured here (not in DECISIONS): no named alternative was resolved this session and the variable isn't load-bearing until link handling is technically settled. Forward work is in BACKLOG ("Configurator — derive `account_link` example from the business-name input").
- **Order updates stages** (shown when Order updates is checked, indented `pl-7`): the first **WorkflowCategory** in the corpus — `categorySubs(category)` returns the seven ordered stages of the order lifecycle: Order confirmation / Processing / Shipping confirmation / Out for delivery / Delivered / Return initiated / Refund processed. 7 messages, 21 tone variants (3 tones each). Same row-click + `?`-icon tooltip pattern as Verification and Account events — the categories panel renders stages and subs identically (the corpus shape is the discriminant, not the UI). The category is authored but **ships unchecked** at launch and seeds no default-checked stages; only Verification's primary sub is in `DEFAULT_CHECKED_SUBS`, so toggling Order updates on does not auto-check any stage. The category introduces six SDK-payload-sourced variables: `order_number` (developer-supplied identifier), `tracking_link` (carrier-domain URL — UPS/USPS/FedEx well-known to carriers; public shorteners discouraged), `estimated_delivery` (short day-name or relative phrase; budgets for "Thursday", not a long date), `return_link` (developer's own returns UI on their own domain — RelayKit does not shorten or host it, same shape as `account_link`), `refund_amount` (sized for partial-refund phrasing like "$40 of $50"), and `card_type` (human-readable card label like "Visa credit", never a card number — no PII per D-393). `triggerCue` strings on each Stage match `audits/research/2026-05-16/order-updates.md` §2 verbatim and surface no UI today — they document the trigger semantics for workspace authoring and the future configurator IA pass.
