...
The customer-experience side of the marketing surface — what visitors see at relaykit.ai before they engage — is described against the marketing-site implementation in `PROTOTYPE_SPEC.md` §"Production Marketing Site — relaykit.ai". The condensed shape: a **v10 marketing home at `/`** (hero with a rotating category line, a "why RelayKit" recognition compare, how-it-works, an embedded live configurator window, what-we-handle, an AI build-spec section, a real-phone "prove" section, pricing, and a closing CTA), the **free message configurator at its own route `/messages`**, legal docs (Terms, Privacy, Acceptable Use), a waitlist signup flow (`/start/verify` + `/start/get-started`), a "we'll be ready soon" `/signup` placeholder, and a blog (`/blog/*` — see §3.3). The site runs **dark mode as the default** with a single **metallic-gold accent** over the monochrome base (D-427); the top nav carries a mobile hamburger menu below the `sm` breakpoint. The free configurator now lives at `/messages` (D-428) — a standalone authoring tool positioned as a free product in its own right (MD-21); the marketing home at `/` embeds the **same live `ConfiguratorSection`** in a clipped window and routes visitors to it via an "Open the configurator" link, so there is one tool, not a fork. `/messages` opens with a dismissible four-step **"Quick start"** orientation strip (Details → Select → Personalize → Edit; D-429) shown at rest above the tool. The configurator is a lightweight authoring tool (Session 106, D-414), not just a preview. In the upper-right above the tone pills, an elig section (reworked Session 123, D-425) asks the visitor two label-less questions — what industry their app serves, and where in that industry they sit. **The message authoring below stays live for every industry the visitor can pick** — the configurator is a complete free tool, never gated by industry (MD-21, D-425). Most picks (generic indie SaaS) land "clear" and nothing extra surfaces. Some land "conditional," where a quiet rules card shows three plain-language rule bullets ("keep balances out of texts," "no specific transactions," and so on) so the visitor shapes their own messages right. Some land in a "not yet" bucket — RelayKit doesn't send that category at launch — and the same rules card carries a small footer line, "RelayKit doesn't send this category. Request it.", whose link opens a one-field email form so we can prioritize what to enable next (the signal is tagged by sub-vertical). A handful of industries RelayKit won't serve at all (cannabis, firearms, vape, adult content, adult dating, surveillance, and CPaaS-adjacent messaging platforms) appear in the sub-vertical dropdown as greyed-out, unpickable items under a "Not supported" heading — the visitor sees they exist but can't select them, and nothing in the tool changes. The multi-tenant question earlier versions asked up front was removed (D-426) — it was friction for a capability that isn't part of launch. Selections persist as the durable handoff to the future onboarding wizard — what a visitor declares here flows forward into registration without re-entry per the continuity-of-intent principle (MASTER_PLAN principle #7). A visitor picks categories, types their business name into a top-of-page input that drives every identity-token preview, opens a "Variables" form on each category to fill in their own variable values (their order number format, their ticket-link domain, their reschedule URL — every preview in the category updates live as they type), hand-edits individual message bodies or pins a per-message tone, and authors their own custom messages via "+ Add message". Double-clicking any variable chip in a message preview jumps the visitor straight to where that variable is edited — for the business-name token, the page scrolls to and focuses the top-of-page input; for any category-specific variable, the Variables form opens on that category with that variable's input focused and the cursor at the end of any existing value. Everything they author persists across sessions and is structured to carry into the workspace at "Start building" — the configurator is now the front of the workspace persistence spine. A "Copy" button emits each visible message as a personalized example + the raw `{{token}}` template, in the active tone, respecting all per-message edits; both audiences (visitors who copy-and-use elsewhere; eventual RelayKit users who wire the template into the SDK) are served from the same output. A kebab next to Copy ("Reset all to defaults") wipes all authored content back to the seed; a kebab on each category heading ("Reset to defaults") wipes one category. Read cards carry a small amber triangle warning when a message renders over 160 characters ("counts as 2 messages" — non-blocking, the visitor can still copy and use it); the edit card surfaces the full compliance issue list, separating warnings (amber, Save still enabled) from blockers (red, Save disabled). The corpus driving all of this is the same nine authored categories from Session 103, each expanding to a flat list of selectable messages (Verification: Verification code, Confirmation code, Recovery code, Login code; Appointments: Confirmation, Reminder - distant, Reminder - proximate, Reschedule confirmation, Cancellation confirmation, No-show follow-up, Post-appointment; Order updates: Order confirmed, Order processing, Order shipped, Out for delivery, Order delivered, Return started, Refund processed; Customer support: Ticket received, Agent assigned, Agent response, Resolution notification, CSAT follow-up, Proactive outreach, Service status alert, Account issue resolved; Team alerts: Shift scheduled, Shift reminder, Shift change, Shift cancellation, Shift start, System alert, Escalation ping, On-call page, Service-level alert; Account events: Payment failed, Trial ending, Subscription confirmed, New device sign-in, Account suspended; Community: Live event reminder, Event invitation, Moderation update, Member milestone, Community announcement, Welcome, First action, Resource pointer, Week 1 check-in; Waitlist: Joined, Position update, Almost up, Your turn, Grace expiring, Missed; Marketing: Promotional offer, Product launch, Re-engagement, Event invitation). No category renders as "Coming soon" any longer (Marketing renders last in the panel). The home-page configurator's former "Recommended combinations" preset dropdown was removed Session 103 (D-411). Page-load lands with Verification pre-selected and all 4 Verification message cards rendered (D-409); toggling any other category cascades the same behavior. Below the message stack sits a full-width "Copy messages" CTA (the Session 122 reframe replaced the prior "Get early access" waitlist button here), and directly beneath it a one-line point-of-use disclaimer — "Not legal advice — you're responsible for consent and compliance. See our Terms." — making explicit that the free tool's output is a compliant-ready starting point, not a compliance guarantee (D-424). At the site level, a browsewrap line in the footer ties Visitors to the Terms, AUP, and Privacy Policy, and those documents now explicitly reach anonymous Visitors using the free tool without an account — the Terms define the Website and Authoring Tool, fold them into "Service," and add a §4A stating the tool's output is unenforced guidance, not a guarantee (D-431). The site's other "Get early access" CTAs still open the in-page early-access waitlist modal (see §3.4).
