...
The customer-experience side of the marketing surface — what visitors see at relaykit.ai before they engage — is described against the marketing-site implementation in `PROTOTYPE_SPEC.md` §"Production Marketing Site — relaykit.ai". The condensed shape: a 6-section home page (hero, configurator, build it, test it for real, pricing + paperwork, closing CTA), legal docs (Terms, Privacy, Acceptable Use), a waitlist signup flow (`/start/verify` + `/start/get-started`), a "we'll be ready soon" `/signup` placeholder, and a blog (`/blog/*` — see §3.3). The configurator on the home page is the central conversion surface — customers pick categories and tones and see their messages render live. It is driven by the typed message-library corpus: at launch Verification, Account events, and Order updates are authored and live (Verification expands to four selectable sub-checkboxes — signup verification, step-up, recovery, 2FA; Account events expands to five — payment failed, trial/renewal upcoming, subscription confirmation, security event, account status change; Order updates expands to seven ordered workflow stages — order confirmation, processing, shipping confirmation, out for delivery, delivered, return initiated, refund processed), while the other six categories preview as disabled "Coming soon". Visitors can also author their own custom messages, and selections persist across visits. The "Get early access" CTAs then open an in-page early-access waitlist modal (see §3.4).
