...
**The no-EIN builder.** A meaningful slice of RelayKit's stated eventual audience — no-EIN indie developers, vibe coders, hobbyists — currently cannot onboard through Sinch's 10DLC path at all, because Sinch's brand registration requires an EIN and offers no sole-proprietor entity type (BACKLOG; active research in `/explorations/no-ein-sole-proprietor-path.md`). This is the sharpest reason the reachable launch audience is narrower than the mission's audience. The honest statement for the foundation: **the launch audience is EIN-holding indie SaaS founders.** The no-EIN builder is a real future customer gated on a resolved onboarding path (toll-free verification is the live candidate), and should not be marketed to as if they can sign up today.
