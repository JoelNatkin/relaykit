...
/**
 * Preserved on the marketing-site for future category-page consumption.
 *
 * Currently dormant — no marketing-site file imports this component. It
 * was moved here from /prototype/components/catalog/ when the prototype's
 * marketing surfaces (/, /sms/[category]) were retired so the prototype
 * could model app.relaykit.ai cleanly. Intended consumer is the per-
 * category landing page surface that Phase 2a content authoring (D-384)
 * will eventually feed.
 *
 * The prototype original imported `Message` from `@/data/messages`
 * (prototype-only). To keep this file tsc-clean on the marketing-site
 * side while dormant, a minimal local `Message` interface is inlined
 * below — only the fields actually read by this component
 * (`categoryId`, `expansionType`). When this component is wired up by
 * Phase 2a, the type gets reconciled against the canonical message-
 * source shape per D-381 (single canonical message-source-data store).
 *
 * One other small departure from the prototype original: a `website`
 * prop and its derived `displayUrl` local existed but were never read
 * by the component body. Both were dropped here so MS's stricter eslint
 * config stays clean. Phase 2a can re-add the prop when actually wiring
 * a website value into the form preview.
 */
