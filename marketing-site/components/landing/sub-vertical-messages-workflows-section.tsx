...
// Generic, data-driven "messages" section (Phase 1C A2). A prop-driven copy of
// app/for/developer-tools/messages-workflows-section.tsx — one heading, one set
// of controls (business name + tone), and a Workflows / All messages toggle.
// Workflows (default) leads with the entry's curated workflows; All messages
// drops to the full 9-category browser. Both views are driven by the same
// business name (shared via the configurator store) and tone, so switching the
// toggle keeps the visitor's context. The browser renders chromeless — this
// component owns the heading + controls, so there's no duplicate chrome (D-436
// reuse). The dev-tools static page keeps its own hardcoded copy untouched.

const TONES: VariantTone[] = ["Standard", "Friendly", "Brief"];

type View = "workflows" | "messages";

export function SubVerticalMessagesWorkflowsSection({
  entry,
}: {
  entry: SubVerticalLanding;
}) {
    ...
    >
      <div className="max-w-2xl">
        ...
        <h2 className="mt-4 text-3xl font-bold tracking-tight text-text-primary sm:text-4xl">
          {entry.name} messages. And all of the others.
        </h2>
