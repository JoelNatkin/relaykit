...
1. **Signup phone verification (PRIMARY)** — the developer's user creates an account; the developer wants to verify they own the phone number they registered. Distinct from login 2FA — this is one-time phone-ownership proof at account creation, regardless of whether SMS is later used for auth. The most-shipped SMS-verification use case in indie SaaS that survives the TOTP/passkey transition. Single-message OTP send + verify. Intake question: "What does your user see right before they get this code?" (anchors the message to a recognizable onboarding moment.) **Per D-389, Sub-use 1 also ships a paired welcome template — same trigger (account creation), authored alongside the OTP template.**

2. **Login 2FA with advisory (SECONDARY)** — the developer's user is logging in and the developer wants SMS as a second factor. Path A vocabulary use case, supported with explicit advisory framing because leading SaaS references treat SMS-2FA as last-resort. Same template plumbing as Sub-use 1; the difference is the context the user sees the message in. Critical: advisory wording — "SMS is the most common second factor **but** the least secure — consider TOTP or passkeys for primary 2FA" — surfaces in the configurator UX when this sub is picked, never in the message body itself.
...
**Sub-use 3 context-line guidance (per Session 93 review):** when an action-specific context line is included ("Confirm withdrawal of $X"), it must be factual and short. No verbs like "secure," "protect," or "confirm" carrying marketing implication. Soft target: keep the total message (code + expiry + context) single-segment (~140 chars or less), which works out to ~40-60 chars for the context line in practice. No hard char limit — developers' triggering actions vary in shape, and a single-segment target gives the reason without forcing a rigid cap.

**Advisory wording for Sub-use 2 (configurator surface, not message body):** "SMS is the most common second factor but the least secure — consider TOTP or passkeys for primary 2FA." Surfaces when the developer picks "Login 2FA" in the configurator. Final wording lands during configurator UX work; this is candidate copy.
...
- **Advisory wording for Sub-use 2** — the configurator UX needs one specific sentence when "Login 2FA" is selected. **DEFERRED** to configurator UX work. Candidate copy captured in §3 above: "SMS is the most common second factor but the least secure — consider TOTP or passkeys for primary 2FA." Final wording lands during configurator UX implementation.
- **Onboarding / Welcome boundary** — **RESOLVED per D-389.** Welcome folds into Verification as a paired follow-up template under Sub-use 1 (signup phone verification). Same trigger, dominant indie SaaS pattern is paired-with-OTP. Standalone welcome (no preceding OTP) rare enough not to need a dedicated home at launch; if post-launch demand surfaces, Account events absorbs it.
- **Account events / Subscription lifecycle adjacency** — **RESOLVED.** Account events ships as the 9th launch category (its own research file exists, classification confirmed discrete). Security event sub (Account events Sub 4) owns password-changed / new-device-signin / suspicious-activity triggers. Verification stays focused on the OTP send/check primitive; no overlap.
- **Sub-use 3 context-line length policy** — **RESOLVED.** Soft guidance, not a hard limit. Keep total message single-segment (~140 chars total); context line ~40-60 chars in practice. Captured as authoring guidance in §3 above.
- **Recovery flow paired messaging** — Sub-use 4 (account recovery) sometimes ships with a paired notification to the original auth channel ("a recovery code was sent to +1***-****-1234"). **DEFERRED** to Phase 8 integration guidance. The SMS RelayKit authors is the recovery code itself; the paired email/in-app notification is the developer's responsibility, and the right place to document the pattern is in the integration docs that ship with the SDK (AGENTS.md / per-builder guides), not in this research file.
