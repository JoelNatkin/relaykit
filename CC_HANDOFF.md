...
New order — Hero · StatusBand · `#rules` (Paperwork) · `#configurator` (Messages) · `#variables` · `#build` (AiSection) · `#test` (Prove) · `#how` (How-it-works) · **`#numbers` (NEW)** · `#why` (Recognition/"The problem" accordion) · `#pricing` · `#join`. The six-row compliance accordion moved from position 3 → 10 so the page opens on product, not friction. Verified the prerendered id sequence matches. (Order revised within Session 136 per PM — Paperwork pulled to position 3, Numbers moved to 9, Build/Test ahead of How-it-works.)
...
- Prerendered home id sequence = rules → configurator → variables → build → test → how → numbers → why → pricing → join (Hero/StatusBand have no id). Numbers copy + stats present.

## Carry-forwards (flagged, not done here)
- **Eyeball the revised seams** on the preview: Paperwork now leads the content (position 3, right after the slim StatusBand); the Numbers stat block sits at position 9 between How-it-works and the demoted Recognition accordion. Confirm the new rhythm reads right.
