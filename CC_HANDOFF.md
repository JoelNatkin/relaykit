...
- **Related chips** link to `/messages` for v1 — the sibling `/for/*` pages don't exist yet; wire real cross-links when the 2nd/3rd sub page ships.
...
- **D-436 follow-ups:** a formal `urlSlug` field on the constraints `SubVertical` shape + a dynamic `/for/[sub]` registry land with the 2nd/3rd page (v1 keeps the short slug as a page-level constant). Sibling `/for/*` pages (Identity/SSO, Helpdesk, E-commerce, Subscription boxes) — Related chips await them.
