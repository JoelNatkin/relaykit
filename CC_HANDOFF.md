...
9. **Variable rendering grammar — deferred from Session 34.** Testing surfaced template/variable-data mismatches, e.g. `{service_type}` resolves to "Beauty & wellness appointments" (title-cased, pluralized) and renders mid-sentence as `"...your Beauty & wellness appointments booking..."` or collides with the static word *appointment* in templates (`"{service_type} appointment is confirmed"` → `"Beauty & wellness appointments appointment is confirmed"`). Root cause: variable display forms are authored independently from the templates that consume them. This is a template-authoring/variable-semantics problem, not a rendering bug. Candidate for a new D-number next session — proposal: variables store a canonical base form and the renderer (or authoring tool) handles case + pluralization per insertion context.
