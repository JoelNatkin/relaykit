...
Alternative test. Can you name the specific alternative being rejected and why? → If no real alternative, it's a working preference, not a decision.
...
Implementation note (2026-04-28): legal-doc entity references use "RelayKit LLC, a South Carolina limited liability company (formerly Vaulted Press LLC)" in declaratory openings, "RelayKit LLC (formerly Vaulted Press LLC)" in contact/footer blocks, and bare "RelayKit LLC" in in-prose mentions. "Formerly" framing chosen over "d/b/a" because the SC SOS amendment certified 2026-04-09 makes RelayKit LLC the operative legal name with Vaulted Press LLC as the prior name — not a d/b/a relationship.
...
⚠ Superseded in part by D-370: Universal primitive renamed from verifyCode() to the sendCode+checkCode pair on every namespace. D-360's architectural intent (OTP included with every vertical) preserved.
...
**D-366 — Marketing site hosted on Vercel, not Cloudflare Pages** (Date: 2026-04-28)
The `/marketing-site` Next.js project at `relaykit.ai` is deployed to Vercel (not Cloudflare Pages). Cloudflare is retained only for DNS — it does not process customer data. D-05's "compliance sites on Cloudflare Pages" applies to the per-customer compliance site surface (Phase 5 msgverified) and does not extend to the marketing site, which is a distinct surface with different deploy/build characteristics.
**Supersedes:** none
**Affects:** `/marketing-site`, vercel.com project, DNS records at Cloudflare, legal docs (Privacy §3.1 service providers, §5 infrastructure-security bullet — both updated in `024f1c9` to reflect Vercel hosting; tracking-doc infrastructure-change note in `docs/LEGAL_DOC_DEFERRED_CLAIMS.md`).

**D-367 — Legal docs describe outcomes and obligations, not architecture** (Date: 2026-04-28)
Legal docs (Terms, Privacy, AUP) name what users need to know — their messages get sent or blocked, with reasons — not the internal mechanism that produces those outcomes. Specific component names ("Compliance Proxy"), pipeline stages, runtime-vs-authoring-time distinctions, and named subsystems are implementation detail and do not appear in legal copy. This principle applied retroactively in `8351b32` to retire "Compliance Proxy" terminology across all three legal docs in favor of outcome-describing language ("RelayKit enforces opt-outs," "messages are blocked when..."). Going forward, any architectural shift can occur without legal-doc impact as long as the outcomes and obligations remain accurate.
**Supersedes:** none
**Reasoning:** Related to but distinct from D-279 — D-279 covers WHERE compliance enforcement happens (substantially shifting to authoring time per the website-as-authoring-surface direction); D-367 covers HOW we describe compliance enforcement in legal docs. The two work together: D-279's architectural shift was the immediate trigger for the terminology retirement, but the principle stands independent of any specific architecture and protects future legal-doc work from churn whenever internals change.
**Affects:** `/marketing-site/app/{terms,privacy,acceptable-use}/page.tsx`, future legal-doc edits, `docs/LEGAL_DOC_DEFERRED_CLAIMS.md` architectural-terminology note.

**D-368 — Branch-and-preview workflow for production-facing surfaces** (Date: 2026-04-29)
Production-facing surfaces (marketing site today, future app and dashboard) are deployed via merge to main. Substantive changes go through a feature branch first, where the host platform's preview deployment serves as the staging gate. PM reviews the preview URL before approving merge. Trivial changes (typos, comment-only edits, doc reorgs that don't touch user-facing content) may push directly to main. Branch naming follows commit prefix convention: feat/short-name, fix/short-name, docs/short-name, chore/short-name. /prototype is local-dev only — no production deployment, no preview URL needed. /sdk previews are tested via local install (npm pack + install in a test project). /api deployment target is TBD per Phase 5; this decision generalizes to it when chosen.
**Supersedes:** none
**Reasoning:** Production-facing surfaces benefit from a preview gate even on small teams; the cost is one branch + one merge, the benefit is no rollbacks-from-production for issues a preview would have caught. Vercel auto-deploys every non-main branch to a unique URL, so the staging surface is free.
**Affects:** /marketing-site workflow, future /app and /dashboard workflows, PM_PROJECT_INSTRUCTIONS.md, CLAUDE.md.

**D-369 — OTP validation in launch scope** (Date: 2026-04-30)
Server-side validation of one-time passwords ships at launch as a first-class capability, not a post-launch enhancement. RelayKit owns code generation, hashed storage with TTL, attempt tracking, and rate-limit enforcement. The SDK exposes `checkCode(phone, code)` returning a structured result that lets developers render appropriate error states. This makes "OTP that just works" truthful — developers integrate one SDK and get send + verify, not send-only with a bring-your-own-validation gap.

The decision rejects the alternative of shipping send-only at launch and deferring validation to a post-launch phase. That alternative was viable on narrower scope grounds (smaller launch surface, less backend infrastructure) but fails the strategic test: it makes the "Verification included" marketing pillar dishonest at launch (developers still have to build verification themselves) and weakens the OTP-led beta MVP plan (beta users can't demonstrate the full flow). Send-only OTP is meaningfully weaker than competitors (Twilio Verify, Stytch, Auth0 all ship send+check); shipping it and back-filling the validation later concedes the strongest competitive angle for short-term scope reduction.
**Supersedes:** none (extends D-360, which named OTP as a universal primitive but left rate-limiting "TBD" and never specified validation infrastructure).
**Reasoning:** The strategy work this session surfaced that "OTP as included feature" is RelayKit's strongest marketing pillar and the natural anchor for a beta MVP. Both depend on validation working, not just sending. The cost of shipping validation in launch scope (server endpoint, hashed code storage, rate-limit infrastructure) is bounded; the cost of shipping without it is either an honest-but-weaker pitch or a marketing claim that fails on first contact with developer scrutiny.
**Affects:** MASTER_PLAN.md Phase 6 (vertical hydration extends to include validation infrastructure). New endpoint POST /v1/verify/check in /api. New Postgres table for hashed code storage. Rate-limit infrastructure (per-phone, per-API-key, per-account layers). New SDK method checkCode on every namespace per D-370. Dashboard Verification panel scope (launch). Beta MVP scope (per existing BACKLOG entry). VERIFICATION_SPEC.md as the canonical source for endpoint contract, storage schema, rate-limit thresholds, and validation response shape.

**D-370 — Universal sendCode+checkCode SDK pair on every namespace** (Date: 2026-04-30)
Every vertical SDK namespace exposes both `sendCode(phone, { code })` and `checkCode(phone, code)`. The verification namespace exposes the pair plus its more specific methods (`sendPasswordReset`, `sendNewDevice`). The pair is symmetric across namespaces — `relaykit.appointments.sendCode()` and `relaykit.appointments.checkCode()` work identically to `relaykit.orders.sendCode()` and `relaykit.orders.checkCode()`.

The decision rejects two alternatives. First, asymmetric exposure (only the verification namespace has checkCode, other namespaces have only sendCode) — rejected because it forces developers using OTP-as-included in a non-verification vertical to switch namespaces just to validate, which breaks the "OTP just works alongside your appointments code" pitch. Second, keeping D-360's original name `verifyCode()` as a single method that only sends — rejected because the name implies validation but the behavior was send-only, creating an obvious confusion. The symmetric pair eliminates both problems.
**Supersedes:** D-360 partially. D-360 specified a universal primitive named verifyCode() that sent codes; this decision renames and pairs that primitive into sendCode+checkCode. The architectural intent of D-360 (OTP as a feature included in every vertical) is preserved; the naming and shape of the SDK surface is updated.
**Affects:** SDK namespaces in /sdk/src/namespaces/ — new sendCode and checkCode methods on every vertical namespace. Existing verification.sendCode retained; new verification.checkCode added. Type definitions in sdk/src/types.ts. SDK_BUILD_PLAN.md. AGENTS.md and SDK README (Phase 8). VERIFICATION_SPEC.md for the contract.

**D-371 — Verification message customizability matches canon-template pattern** (Date: 2026-04-30)
Verification message bodies are editable by the developer with compliance gates that match the existing canon-template pattern (per D-356): {code} placeholder is required and immutable, opt-out language is required, character cap enforced, AI polish available via the same "Ask AI: polish my edit" affordance other messages use. No special-casing for verification — the message is treated as a canon template with one additional required placeholder.

The decision rejects two alternatives. First, fully locked verification copy (RelayKit ships canonical wording, no edits) — rejected because the "RelayKit:" or generic prefix doesn't match the developer's brand, and "OTP that just works" doesn't require "OTP that you can't customize." It also creates an inconsistency with how every other message type works. Second, fully editable copy with no {code} requirement — rejected because a verification message without the placeholder is a broken integration that can't be detected until a real user fails to verify. The compliance gate is cheap and prevents a class of silent bugs.
**Supersedes:** none
**Reasoning:** Consistency with the existing message-authoring surface beats verification-specific carve-outs. Developers already understand the edit-with-compliance-gates pattern from canon templates; verification inheriting the same model means no new learning surface, no new dashboard chrome, no special documentation. The {code} requirement is the only new gate, and it's mechanically obvious (the message has to send a code).
**Affects:** PROTOTYPE_SPEC.md Verification panel section. Compliance gate logic in the message editor (/prototype/lib/editor/) — extend the existing gate set with {code} required-and-immutable for verification templates. The API template registry's verification entries.

**D-372 — Three-layer product model: TCR categories, SDK namespaces, audience-packs** (Date: 2026-05-03)

RelayKit's product is structured as three distinct layers, each with its own source of truth. (1) TCR categories — carrier-side, immutable, defined by The Campaign Registry. Standard set of 10 plus Special vetted set of 12 (enumerated in VERTICAL_TAXONOMY_DRAFT §1). Every campaign RelayKit submits configures within this fixed taxonomy. (2) SDK namespaces — RelayKit's developer-facing API. Eight namespaces per D-273 (`appointments`, `orders`, `verification`, `support`, `marketing`, `internal`, `community`, `waitlist`) plus `relaykit.send()` escape hatch (D-275) and top-level consent functions (D-274). Higher Education will be added as a ninth namespace per VERTICAL_TAXONOMY_DRAFT §3 settled call. (3) Audience-packs — curated configurations (templates, intake flow, marketing voice, landing page surface) for specific business types, composing across multiple SDK namespaces within the fixed TCR taxonomy. Indie SaaS is audience-pack #1 per MARKETING_STRATEGY MD-11. The three layers do not collapse: a single audience-pack composes across multiple namespaces, and a single namespace can serve multiple packs.

**Supersedes:** none

**Reasoning:** The three-layer separation is the load-bearing structure that lets RelayKit (a) configure audience-packs for specific business types without rebuilding the SDK or hitting carrier-side constraints, (b) add new audience-packs as separate work from new SDK namespaces, and (c) keep the carrier-side TCR taxonomy as the immutable bottom layer every pack composes within. Recording as a D-number ensures future architecture decisions respect the layer separation rather than collapsing layers (e.g., shipping audience-pack-specific SDK methods, or treating SDK namespaces as ready-made audience-packs).

**Affects:** MASTER_PLAN.md §0/§1/§16/§17 (committed v1.5); MARKETING_STRATEGY.md MD-9 / MD-11 (committed); VERTICAL_TAXONOMY_DRAFT.md §0 (added in commit 2 of this session); Phase 5/6 architecture decisions.

**Superseded by D-385 (2026-05-12)** — the three-layer model recorded here is replaced by the two-layer model in D-385. RelayKit's product is structured as TCR categories + SDK namespaces, not three layers; audience-packs as an architectural concept are excised per D-385.

**D-373 — Two-layer pumping defense architecture (app-side guidance + pipeline-side safety net)** (Date: 2026-05-03)

RelayKit's pumping defense architecture is two-layer: (1) **Layer A — app-side defense**, prescribed via integration-time guidance in AGENTS.md, the integration prompt template (D-331), and per-builder guides; the customer's AI tool wires these defenses into the customer's app during integration. The seven prescribed practices: bot detection (Cloudflare Turnstile / hCaptcha), per-IP edge rate-limiting, per-account-state rate-limiting, honeypot fields, phone validation pre-send, server-side gating, namespace-specific surface guidance. (2) **Layer B — pipeline-side defense**, namespace-agnostic, runs in RelayKit's message pipeline on every send: country allow-list (US-only default), per-destination rate limit, per-customer rate limit, premium-prefix block list. Both layers are load-bearing — neither alone is sufficient. Canonical detail: `docs/SECURITY_DRAFT.md` §3.

**Supersedes:** none

**Reasoning:** Three alternatives rejected. (a) Pipeline-side-only defense (Layer B alone) — rejected because it misses the integration-time leverage that makes app-side defenses load-bearing in customer code, and concedes the structural differentiation against usage-based incumbents. (b) Carrier-side fraud detection alone (Twilio Fraud Guard model) — rejected because it's reactive (catches attacks after sends fire and bills accrue), and because it's a paid add-on framing that contradicts RelayKit's "defense by default" posture. (c) Indie devs handle defense themselves — rejected because indie devs don't currently know they should fear pumping (per MARKETING_STRATEGY MD-14 awareness-gap risk), and the integration-time moment is the only realistic leverage point for prescriptive defense; retrofitting is not a thing that happens.

**Affects:** MASTER_PLAN.md v1.6 §2 (working principle), §9 Phase 5 (pipeline-side build), §12 Phase 8 (app-side guidance content); `docs/SECURITY_DRAFT.md` §1, §3 (canonical home); MARKETING_STRATEGY MD-13 (positioning pillar — lands in Wave 1 Commit 5, sequencing-after); MESSAGE_PIPELINE_SPEC future extension (Wave 2); SDK_BUILD_PLAN / AGENTS.md / integration prompt / per-builder guides future extensions (Wave 2).

**D-374 — Manual-monitoring bridge for first 50 customers; automated anomaly detection deferred to post-launch BACKLOG** (Date: 2026-05-03)

Automated anomaly detection (velocity-vs-baseline + destination-diversity signal, layered response from log → alert → auto-action) does not ship at launch. RelayKit-side manual monitoring (billing dashboard signals, per-customer destination distribution review, velocity vs onboarding-stated projections) substitutes during the first-50-customers window. Manual intervention on detected attacks. Graduation to automated anomaly detection is post-launch BACKLOG work; graduation triggers include customer count exceeding manual monitoring capacity, accumulated real-traffic data sufficient to set thresholds without guessing, and customer-tier-mix maturing enough to need per-tier baseline differentiation. Canonical detail: `docs/SECURITY_DRAFT.md` §3.4.

**Supersedes:** none

**Reasoning:** Two alternatives rejected. (a) Ship MVP automated anomaly detection at launch — rejected because launch-time threshold tuning would be guessed against zero real customer traffic, false-positive risk against legitimate spikes (campaigns, Product Hunt launches) is real, and the marginal value over manual monitoring at first-50-customers scale is low. The marketing pillar can be stated honestly via build-in-public graduation narrative (MARKETING_STRATEGY MD-15, lands in Wave 1 Commit 5) instead of overpromising automation that doesn't yet exist. (b) Ship no monitoring at all — rejected because Layers 1–4 alone are static-rule defenses; opportunistic scanners can adapt around them, and manual monitoring is cheap insurance that catches the residual cases.

**Affects:** MASTER_PLAN.md v1.6 §9 (manual-monitoring bridge), §16 (deferred), §17 (manual monitoring scaling-limit risk), §18 (graduation triggers open question); `docs/SECURITY_DRAFT.md` §3.3 (anomaly detection deferred row), §3.4 (manual monitoring bridge canonical home); MARKETING_STRATEGY MD-15 (build-in-public graduation narrative, lands in Wave 1 Commit 5); BACKLOG entries for graduation work (Wave 1 Commit 5).

**D-375 — Marketing-site replicates the prototype's Tiptap message editor; no shared workspace package** (Date: 2026-05-09)

The Tiptap-based message editor (variable-node, variable-node-view, template-serde, message-editor) is replicated from `prototype/lib/editor/` into `marketing-site/lib/editor/` rather than extracted into a shared workspace package both apps import. The compliance check and variable-scope helpers are similarly duplicated under `marketing-site/lib/configurator/`. Each ported file carries a header comment naming its prototype source and the parity expectation. The two copies are intentionally identical at this commit; future divergence between dashboard and marketing-surface edit experiences must be deliberate and documented rather than incidental — when prototype's editor evolves, the marketing-site copy is re-synced or the divergence is recorded.
...
**D-379 — Home configurator and workspace messages render from a single canonical source** (Date: 2026-05-11)
The marketing-site home page configurator and the prototype/eventual workspace messages page show the same default message set per category at all times — no drift, no divergence. The home page is not a sample or teaser; it is the full product the customer will use, rendered as a lead-magnet preview surface.
**Supersedes:** none
...
⚠ Partially superseded by D-428: the home-page *placement* claim is narrowed — the full configurator now lives at `/messages`; the home (`/`) shows the **real `<ConfiguratorSection/>`** in a clipped, fixed-height window (a sample *view* of the full tool, not a recreation). D-379's one-source / no-drift *principle* is unchanged and is in fact strengthened — the home embeds the very same component `/messages` renders.
...
**Supersedes:** none
**Reasoning:** The home page is a working preview, not a static demo. Customers need to populate real values to evaluate fit ("does this read like a message I'd send?"). Continuity from home → onboarding → Settings prevents re-entry friction across the funnel. The registration-commit lock matches carrier campaign semantics (TCR brand identity is locked once approved). Additional fields may emerge during Phase 2 research; four is the floor.
**Affects:** `marketing-site/components/configurator-section.tsx`, `marketing-site/lib/configurator/example-values.ts`, prototype `/start/*` intake step ordering and validation, `prototype/app/apps/[appId]/settings/page.tsx`, Phase 5 registration-commit logic.

**D-381 — Single canonical message-source-data store; storage architecture deferred until workspace activates** (Date: 2026-05-11)
The ONE SET principle of D-379 commits us to a single canonical message source. The physical implementation of that source — extracted shared package, mirrored file with sync, third-party storage — is deferred until the workspace messages page activates post-IH-launch, at which point the architectural choice gets revisited against then-current monorepo tooling and surface counts. For IH-launch, only the home page renders the canonical set; "one source" is the principle, with concrete storage architecture chosen when actual two-surface rendering goes live.
**Supersedes:** none
...
Per D-372's three-layer model (TCR categories / SDK namespaces / audience-packs), the Phase 2a configurator content authoring workstream produces templates, intake flow, and configurator surface treatments for the indie SaaS audience-pack (pack #1 per MD-11) — not for the SDK namespace layer. The eight SDK namespaces (D-273), with Higher Education added as the ninth per VERTICAL_TAXONOMY_DRAFT §3 + D-372, ship as audience-agnostic infrastructure regardless of pack. What's pack-specific is which namespaces the pack composes from, the templates within them shaped for the pack's audience, the intake flow guiding builders into the right composition, and the configurator's "recommended combinations" presets. Phase 2a "launch posture per category" framing inherited from pre-D-372 PRDs (and propagated through `audits/PHASE_B_PRIOR_ART_DIGEST.md` findings #1, #2, #4) is a category error against the layer model and is corrected by this decision.

**Supersedes:** none (clarifies D-372 + MD-11 application to Phase 2a)

**Reasoning:** Prevents Phase 2a from collapsing the three layers. Specifically: prevents "which categories ship at launch" decisions at the SDK namespace layer (namespaces ship as infrastructure, separate from pack composition), and frames the actual authoring questions as (a) which namespaces does the indie SaaS pack compose from, (b) what templates does the pack use within each, (c) what intake flow surfaces pack-appropriate compositions, (d) which configurator presets does the pack offer to builders.

**Affects:** Phase 2a research scope; Phase 2b authoring scope; the corrective header on `audits/PHASE_B_PRIOR_ART_DIGEST.md`; the §17/§18/§10 amendments in MASTER_PLAN v1.8.

**Superseded by D-385 (2026-05-12)** — the audience-pack layer this decision operated against is excised per D-385. Phase 2a authors per-category content for the launch audience (indie SaaS founders) at the SDK-namespace layer with audience-specific framing in templates, not at a separate pack layer.

**D-383 — Phase 2a research methodology: 4-source external scan per category, full universe of sub-uses, indie-SaaS-pack fit assessment** (Date: 2026-05-11)

Phase 2a per-category research uses four source types per category: (1) **Competitor surface scan** — how Twilio / Telnyx / Plivo / Sinch's messaging products categorize use cases and what templates they ship; (2) **Indie SaaS practice scan** — what notification sub-uses Stripe, Vercel, Linear, Lemon Squeezy, GitHub, and similar products actually expose; (3) **Starter kit scan** — what SMS sub-uses ShipFast, Supastarter, MakerKit, Vercel/Supabase starter templates expect; (4) **Community signal** — what SMS use cases indie SaaS founders discuss on Indie Hackers, r/SaaS, Twitter / X, AI-tool Discords. Per-category output: full universe of sub-uses (not constrained by prior PRD guesses or current SDK namespace internal lists), indie-SaaS-pack fit per sub-use with evidence, linguistic shape evidence (length / tone / variables / opt-out placement / sequence structure), pack-composition recommendation with reasoning. Process: PM-led research in browser chat with Joel reacting as findings land; multi-chat workstream — Verification and Marketing first per IH launch priority (each likely its own chat); other categories follow per emerging priority. Output feeds Phase 2b per-category template authoring (for the indie SaaS pack), the configurator's "recommended combinations" presets per D-372, and a prioritized post-launch category-and-sub-use backlog.

**Supersedes:** implicit understanding that `audits/PHASE_B_PRIOR_ART_DIGEST.md` was sufficient research input. The digest is internal prior-art mining from Feb–Mar 2026 PRDs that predate D-372's three-layer repositioning and MD-9's audience narrowing; useful as prior-art floor, insufficient as research foundation.
...
**D-385 — Two-layer product model: TCR categories and SDK namespaces; audience-packs deprecated as architectural concept** (Date: 2026-05-12)

RelayKit's product is structured as two layers, not three. (1) **TCR categories** — carrier-side, immutable, defined by The Campaign Registry; Standard set of 10 plus Special vetted set of 12 (enumerated in VERTICAL_TAXONOMY_DRAFT §1); every campaign RelayKit submits configures within this fixed taxonomy. (2) **SDK namespaces** — RelayKit's developer-facing API; eight namespaces per D-273 plus `relaykit.send()` escape hatch (D-275) and top-level consent functions (D-274). The "audience-pack" layer committed in D-372 — curated configurations for specific business types composing across namespaces — has no behavior in the shipped product. The "Recommended combinations" dropdown in the configurator (D-377) is a checkbox preset with no effect on message content, no audience-specific template variation, and no architectural distinction between the audiences it presets. The pack concept is excised from canon as an architectural commitment; indie SaaS founders remain the launch target audience as a marketing-positioning fact (MD-9), not as "pack #1 of a future audience-pack platform."

**Supersedes:** D-372 (three-layer product model) — fully; the three-layer separation it committed to is replaced by the two-layer model recorded here. D-382 (Phase 2a operates at the audience-pack layer, not the SDK namespace layer) — fully; Phase 2a authors per-category content for the launch audience (indie SaaS founders) at the SDK-namespace layer with audience-specific framing in templates, not at a separate pack layer. D-383 (4-source research methodology) and D-384 (CC/PM split) survive without supersession — references within them to "indie-SaaS-pack fit" or "audience-pack composition" are read as referring to the indie SaaS launch audience in light of this entry's timeline.

**Reasoning:** D-372 was committed 2026-05-03 as part of the strategic repositioning amendment (MASTER_PLAN v1.5) and forecast a future where audience-packs would be a meaningful architectural layer. The "Recommended combinations" dropdown shipped in the marketing-site configurator as the only product-surface manifestation of the pack layer, and it carries no audience-specific behavior — same messages, same templates, same compliance machinery across SaaS, Personal services, Real estate, Fitness, E-commerce, Custom, and None. The pack layer as forecast never materialized; the inherited canonical commitment is a category error against product reality. Excising the layer simplifies the strategic model, removes dangling forward commitments (pack #2 gating, future-pack roadmap, pack-namespace composition open questions), and lets the launch positioning stand on the marketing-positioning fact alone. If an audience-bound product layer ever becomes load-bearing — audience-specific templates, intake variations, or surface treatments — that future commitment becomes its own decision then, derived from then-current evidence rather than the 2026-05-03 forecast.

**Affects:** MASTER_PLAN.md (v1.8 → v1.9: §0, §1, §2, §10, §17, §18); MARKETING_STRATEGY.md (MD-11 supersession via new MD-18); PROTOTYPE_SPEC.md (line 180 prose; code identifier `resolvePackId` unaffected per user-facing-vs-internal-naming rule); VERTICAL_TAXONOMY_DRAFT.md (§0 deletion, §5 audience-pack composition bullet removal, v0.2 → v0.3); DECISIONS.md (supersession marks on D-372 and D-382 in the same commit as this entry).

**D-386 — Higher Education deferred from launch** (Date: 2026-05-12)

Higher Education is removed from launch scope as a configurator category and SDK namespace addition. The SDK ships with eight namespaces per D-273 (`appointments`, `orders`, `verification`, `support`, `marketing`, `internal`, `community`, `waitlist`); no ninth namespace is added at launch. The configurator's category list at the marketing-site home is eight categories (Verification, Appointments, Order updates, Customer support, Marketing, Team alerts, Community, Waitlist). Higher Education's status moves to backlog (post-launch revisit).

**Supersedes:** VERTICAL_TAXONOMY_DRAFT §3 settled call "Add Higher Education vertical" — fully; that call is retracted. Any references in D-372 (now superseded by D-385), D-382 (now superseded by D-385), MASTER_PLAN §17 risk row, or VERTICAL_TAXONOMY_DRAFT §2/§3/§5/§6 to Higher Education as a planned ninth namespace are read in light of this entry. The TCR Higher Education category itself remains a factual element of TCR taxonomy (referenced in VERTICAL_TAXONOMY_DRAFT §1 as a Standard category) — this decision does not change that; only RelayKit's commitment to add a Higher Education namespace at launch is retracted.

**Reasoning:** Per Session 81 (2026-05-11) Phase 2a research-scoping discussion, Higher Education was assessed as the highest-effort Phase 2a authoring task with the lowest payoff for the indie SaaS launch audience. Specific friction: zero prior thinking in archive PRDs (every prior PRD enumerates 9 categories with "exploring" as the 9th, never Higher Ed); the only adjacent artifact is a `/src` education industry-gating keyword (not a category); authoring Higher Ed at launch would mean inventing sub-uses, variable model, FERPA-shaped compliance overlay, and TCR mapping simultaneously; universities are not the launch target audience (indie SaaS founders per MD-9). The deferral was assented to in Session 81 ("Sure, defer higher ed") but never recorded as a D-number — this entry backfills the missing record. Adding Higher Education post-launch is straightforward when real customer pull emerges from the higher-ed segment, with the benefit of evidence-driven sub-use definition rather than blank-slate authoring.

**Affects:** SDK namespaces (no change to D-273's eight); VERTICAL_TAXONOMY_DRAFT.md (§3 "Add Higher Education vertical" settled call deleted; §2 mapping table row removed; §5 implementation-impact bullets mentioning Higher Education removed; §6 prerequisite-gate references removed); MASTER_PLAN.md §17 risk row (Higher Education reference removed as part of the broader pack-deprecation rewrite per D-385); BACKLOG.md (existing "Add Higher Education vertical" entry updated to reflect deferral status with this D-number as the deferral record).

**D-387 — Blog ships as in-repo MDX on the canonical domain at relaykit.ai/blog** (Date: 2026-05-15)

RelayKit's blog is built as MDX files inside the existing `marketing-site/` Next.js app and rendered at `relaykit.ai/blog/*` — not on an external publishing platform (Beehiiv, Substack, Ghost, Medium) and not on a separate subdomain (`blog.relaykit.ai`). Posts live as `content/posts/*.mdx`; the rendering pipeline, RSS feed, sitemap, and per-post SEO metadata are part of the marketing-site codebase. The V1 scaffold shipped Session 89 on branch `feat/blog-scaffold`.

**Supersedes:** none

**Reasoning:** The blog's job is an SEO surface for the POST_TOPICS.md corpus and a permanent, founder-controlled archive. An external SaaS would split link equity off the canonical domain, put post styling and data outside RelayKit's control, and create a migration liability if the platform is later dropped. A subdomain dilutes domain authority versus a path on the root domain. In-repo MDX keeps every post on `relaykit.ai`, under version control, styled with the site's own design system, with `rel=canonical` discipline owned in-house — the corpus's "canonical home" that POST_TOPICS.md §7 names as a prerequisite. The cost is that authors write MDX and posts ship via git rather than through a CMS UI; for a single-author technical blog that cost is acceptable.

**Affects:** `marketing-site/` (new `app/blog/*` routes, `lib/blog/*`, `components/blog/*`, `content/posts/*`, `app/sitemap.ts`, `app/robots.ts`; `app/layout.tsx` gains `metadataBase`); PROTOTYPE_SPEC.md ("Production Marketing Site" section gains a Blog subsection); PRODUCT_SUMMARY.md §3 (blog added as a pre-auth surface); POST_TOPICS.md §7 prerequisite satisfied.

**D-388 — Blog content is organized by cluster as the primary axis; lane is secondary metadata** (Date: 2026-05-15)

The blog's primary organizing axis is the topical cluster (11 clusters per POST_TOPICS.md §4). Clusters get their own index pages and URLs (`/blog/cluster/[name]`) and are surfaced as a post's clickable category. The narrative lane ([D]emand / [S]upply / [R]etrospective / [W]orldview) is secondary metadata — recorded in frontmatter and shown on posts as quiet text, but given no index pages, no filter UI, and no URLs.
...
⚠ Superseded by D-409: per-message default-off carve-outs are deleted in favor of a corpus-wide cascade rule (category on ⇒ every message on); milestone now auto-checks alongside Community when Community is toggled. The protective intent (milestone shouldn't auto-send to early-stage communities) returns as a separate decision when/if the BACKLOG Pri 1 "Opt-out risk tagging for message templates" feature lands.
...
⚠ Amended by D-427 (2026-06-06): a metallic-gold chromatic accent (#E0B010) is introduced over the monochrome base as the marketing-site's single brand accent, applied across the actionable + signature surfaces (CTAs, the home configurator-section link, selected checkbox, selected tone pill, eyebrow dots, progress bars, paperwork icon tint, resolve line, code identifier highlight, pricing ticks, "Invited" status dot). D-405's monochrome base stays operative for everything not listed there; only its "no chromatic accent" absolute is narrowed. The selected **category** checkbox returns to a chromatic (gold) fill + dark-ink glyph for its checked state (message-level checkboxes stay neutral), reversing the 2026-05-19 checkbox amendment for the category checkbox only. D-405's semantic error/warning/success colors are unchanged and now also carry status/friction meaning on the marketing surfaces (green = Delivered/Verified, red = friction markers).
...
## D-409 — Configurator category toggle cascades to all messages; per-message defaults removed

**Decided:** 2026-05-21 (Session 101)

**Decision:** Category check-state in the home-page configurator cascades to every message inside the category. Toggling a category ON sets `checked=true` on every message in it; toggling OFF sets every message to `checked=false`. The page-load seed (`seedState`) checks every message of `DEFAULT_CHECKED_CATEGORY` ("verification") and unchecks every message of every other category. The "Verification only" preset reset (`selectPreset`) does the same — every Verification message checked, every other category's messages unchecked. The prior per-message-default mechanism (`DEFAULT_CHECKED_MESSAGES`, a `Record<string, string[]>` of default-checked message IDs) is deleted; seed and preset both derive from `DEFAULT_CHECKED_CATEGORY` alone. `STATE_VERSION` bumps 2 → 3 so V2 persisted state drops silently and returning visitors re-seed under the new default. Tone overrides and visitor custom messages survive the cascade.

**Why:** Resolves the Session 99 / Session 100 carry-forward "authored-but-unchecked category-default behavior — should an authored category auto-suggest a primary message when toggled on?" The answer chosen is no primary, all messages. Four reasons compose: (1) page-load now lands a populated preview pane — for the Indie Hackers launch, 4 Verification message cards render immediately, making the lead-magnet feel substantial rather than sparse; (2) the prior behavior left a visitor confused when they toggled a category on and nothing appeared in the preview pane (the messages were category-checked but each message-checkbox was off); (3) the cascade is the simpler mental model — a category being "on" means its messages are on; (4) removing `DEFAULT_CHECKED_MESSAGES` deletes a bookkeeping list that a future contributor would have to remember to update when a new message is authored, replacing it with a single source of truth (the corpus + `DEFAULT_CHECKED_CATEGORY`).

**Rejected alternative:** Preserve per-message default overrides — specifically the D-397 carve-out where Community's milestone message ships default-off when Community is toggled on. Rejected on the same simplicity grounds: any carve-out reintroduces a bookkeeping list and creates UX inconsistency (most messages auto-check on category toggle, but specific ones don't, with no visible cue why). A developer who wants milestone off can simply uncheck it after toggling Community on — same effort, no special configurator-side state. If a real UX need for per-message defaults re-emerges later (e.g. via the BACKLOG Pri 1 "Opt-out risk tagging for message templates" feature), it becomes a new decision then with concrete evidence.
...
## D-427 — Metallic-gold accent system over the D-405 monochrome base (gold accent + D-405 semantic status colors)

**Decided:** 2026-06-06 (Session 127)

**Decision:** RelayKit introduces a metallic-gold accent — Metallic Gold `#E0B010`, a real theme token (`--color-gold` / `--color-text-on-gold` / `--color-bg-gold` / `--color-border-gold` in `globals.css @theme`, the same value in both light and dark modes) — over the D-405 Gray Warm monochrome base, as the marketing-site's single chromatic brand accent. Gold marks the actionable and the brand's signature touches: primary CTA buttons (solid gold fill + `#13120E` dark ink, not an outline); the home configurator section's "Open the configurator →" link; selected category checkboxes (gold fill + dark glyph; message-level checkboxes stay neutral); selected tone pills; section eyebrow dots; how-it-works progress bars; paperwork featured-icon backgrounds (gold tint); the "RelayKit handles them." resolve line; the AI code snippet's identifier/method highlight; pricing checkmarks; the pricing **STAGE 1 / STAGE 2** mono labels on the pricing card; and the preview-list "Invited" status dot. Alongside gold, the marketing surfaces use the D-405 **semantic** colors for status/meaning (not brand): success-green for "Delivered" / "Verified" status, error-red for the recognition section's friction markers. Still NOT gilded — they stay monochrome: variable `{tokens}` (neutral `text-variable`), nav/body links, secondary/ghost buttons, body copy, the inline tone-row "Copy", and the pricing price figure (emphasis via weight, not color). All accent colors flow through tokens; no raw hex outside `globals.css`.

**Why:** Gold gives the marketing surfaces one warm, consistent signature that directs the eye to actions and brand touchpoints without reverting to a hue-driven brand. Green/red carry status and friction *meaning* per D-405 (they read as meaning, not brand). Authoring gold as a token keeps every site in one place and lets the deferred dark-mode legibility tuning land in one edit.

**Rejected alternative:** Staying fully monochrome per D-405 as-is (no accent) — rejected as flat for a marketing home that needs focal color. Also rejected: per-mode gold values — the dark-mode "hue reads dark / dark-ink-on-gold is heavy" tuning is knowingly deferred, not solved by a second value now. (This decision first landed scoped to four sites; it was widened in the same session, before merge, to the accent system described above — same conceptual decision, broader application. The earlier "four sites" wording is superseded by this entry.)

**Supersedes:** none — this is an amendment, not an overturn. D-405's monochrome *system* stays operative; only its "no chromatic accent" absolute is narrowed, and the selected category checkbox returns to a chromatic (gold) fill (message-level checkboxes stay neutral; reversing D-405's 2026-05-19 amendment for the category checkbox only). D-405's semantic error/warning/success colors were always permitted by D-405 and are unchanged. D-405 marked with an `⚠ Amended by D-427` line in the same commit.

**Affects:** `marketing-site/app/globals.css` (gold tokens in `@theme`, no `.dark` override — deferral commented); `components/home/section-ui.tsx` (eyebrow dot + gold PrimaryCta); `components/home/{how-it-works,paperwork,pricing,ai-section}.tsx`; `components/home/hero.tsx` + `components/preview-list-mock.tsx` (green/gold status dots); `app/page.tsx` (recognition green/red markers + gold resolve line + the embedded configurator's "Open the configurator →" link); `components/configurator/{tone-pill.ts,category-list.tsx}` + `components/configurator-section.tsx` (selected pill, selected checkbox, Copy CTA — render at `/messages` and in the home's embedded `ConfiguratorSection`).

## D-428 — Marketing home at `/`; the configurator tool moves to `/messages`

**Decided:** 2026-06-06 (Session 127)

**Decision:** `/` becomes a marketing home (hero, how-it-works, configurator window, paperwork, AI, test, pricing, CTA) rebuilt from the v10 design. The full message configurator — the free standalone authoring tool — moves to its own route, `/messages`, with a thin demand-voice header and the real `ConfiguratorSection` (its own go-live/forward CTA intact). The home embeds the **same real `<ConfiguratorSection/>`** in a clipped, fixed-height window (overflow-hidden, the lower categories cropped) with an "Open the configurator →" link to `/messages` — the literal live tool, not a recreation. (An interim hand-built peek shipped in Wave 3 and was replaced by the real embed in Wave 3 review.)

**Why:** A marketing home and a free standalone tool want different surfaces. The home sells the integrated paid product (D-416) and routes interested visitors to the tool; the tool gets a clean, focused URL it can be linked to directly and that reads as a product in its own right (MD-21). Splitting them lets each be built and iterated without the other's constraints, and the clipped embed still demonstrates the live tool on the home without forking it.
...
**Why:** The configurator is a free, learnable-by-doing authoring tool (MD-21). A static, at-rest strip tells a first-time visitor the shape of the work — four steps — without gating interaction or demanding completion. Page-level scoping keeps it off the home's clipped configurator peek.

**Rejected alternative:** A sequential tooltip/coachmark tour — completion drops off sharply past ~step 5, and a forced walkthrough fights a tool that is obvious to operate. A dismissible at-rest strip orients without interrupting.

**Cross-ref:** MD-21 (configurator reframed as a free standalone authoring tool).
...
## D-436 — Sub-vertical landing-page architecture: shared home-component chrome + per-sub sections, self-canonical, short `/for/{slug}` URLs

**Decided:** 2026-06-16 (Session 139)

**Decision:** Sub-vertical landing pages are composed from the live home section components — imported **as-is** for the shared "chrome" (status band, problem, paperwork, build, test, process, price, closing CTA) — wrapped around per-sub sections of two kinds: (a) **authored fresh** each page (hero, moment, details/Q&A, "rest"/secondaries, related) and (b) the **same home component fed sub-specific data** (Messages locked to the sub's dominant category; Variables given a sub-matched example). Build, Test, and Numbers do **not** vary per sub — any mockup tailoring of those reverts to the home copy. Each page is **self-canonical**: its `<link rel="canonical">` points to its own path, never to `/`. URLs are `/for/{slug}` where `{slug}` is a **short, human-curated kebab slug** (e.g. `developer-tools`), NOT the long constraints data slug. The page resolves its sub-vertical data (`name`/`bucket`/rules) from `/lib/constraints` separately, by the canonical data slug (e.g. `developer-tools-api-platforms-infrastructure-saas`); for v1 the short URL slug is a page-level constant, and a formal `urlSlug` field is added to the constraints `SubVertical` shape when the dynamic page registry is built (2nd/3rd page).

**Why:** A system of one-per-sub landing pages must stay maintainable as it grows to dozens. Reusing the home's section components for the shared chrome means a chrome change lands once and propagates; only the genuinely per-sub material (the dominant-story sections + sub-matched data) is authored. Self-canonical URLs make each page its own indexable destination rather than a duplicate of `/`. Decoupling the short URL slug from the long constraints data slug keeps URLs clean and human while preserving data alignment.

**Rejected alternative:** Bespoke per-page sections cloned from the HTML mockups. Rejected — it produces N hand-maintained clones that drift from the live product, and it breaks the one-source principle for the shared chrome (D-379). Also rejected: using the long constraints data slug in the URL — needlessly long and couples the public URL to an internal taxonomy identifier.
