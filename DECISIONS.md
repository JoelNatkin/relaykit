...
**D-236 — Novel message detection as AI compliance trigger** (Date: 2026-03-25)
The deterministic compliance proxy (PRD_09) classifies every outbound message against the customer's registered template set by structural similarity (template shape matching, not exact string). Messages matching known templates pass through with no additional check. Messages that do NOT match any known template — novel messages — are routed to an async AI evaluation layer. The AI assesses whether the novel message is within scope of the customer's registered use case. Three outcomes: (1) Within scope — message delivers normally, template shape added to known set for future matching. (2) Borderline — message delivers but flags in operator attention queue for review. (3) Out of scope — message queued with "compliance review" API response, operator notified. This replaces random traffic sampling. AI cost scales with novelty (rare), not volume (expensive). Deterministic proxy remains the real-time gatekeeper for all objective checks (opt-out language, message type registration, opt-out list, rate limits). AI layer handles the judgment calls that deterministic rules can't.
_Affects: PRD_09 compliance proxy architecture, admin attention queue, API response codes._
