...
⚠ Amended by D-427 (2026-06-06): a metallic-gold chromatic accent (#E0B010) is introduced over the monochrome base as the marketing-site's single brand accent, applied across the actionable + signature surfaces (CTAs, peek link, selected checkbox, selected tone pill, eyebrow dots, progress bars, paperwork icon tint, resolve line, code identifier highlight, pricing ticks, "Invited" status dot). D-405's monochrome base stays operative for everything not listed there; only its "no chromatic accent" absolute is narrowed. The selected checkbox returns to a chromatic (gold) fill + dark-ink glyph for its checked state, reversing the 2026-05-19 checkbox amendment for that state only. D-405's semantic error/warning/success colors are unchanged and now also carry status/friction meaning on the marketing surfaces (green = Delivered/Verified, red = friction markers).
...
**Affects:** `marketing-site/app/globals.css` (gold tokens in `@theme`, no `.dark` override — deferral commented); `components/home/section-ui.tsx` (eyebrow dot + gold PrimaryCta); `components/home/{how-it-works,paperwork,pricing,ai-section}.tsx`; `components/home/hero.tsx` + `components/preview-list-mock.tsx` (green/gold status dots); `app/page.tsx` (recognition red markers + gold resolve line); `components/configurator-peek.tsx` (peek gold sites); `components/configurator/{tone-pill.ts,category-list.tsx}` + `components/configurator-section.tsx` (selected pill, selected checkbox, Copy CTA at `/messages`).
