...
/**
 * Placeholder sign-in landing.
 *
 * Ports the email → OTP form JSX from `prototype/components/sign-in-modal.tsx`
 * (lines 160–251 of the modal at the time of this lift) and wraps it
 * in a standalone page layout — centered card on the page body, no
 * backdrop, no XClose. Reaches this route via the auth-aware redirect
 * at `/` for logged-out users and via the `/apps` auth-guard for
 * unauthenticated workspace visits.
 *
 * Status: placeholder. The auth refactor (Supabase social providers,
 * UI redesign) is a separate future workstream; when it lands, both
 * this page and the dormant SignInModal component will be retired
 * together. Behavior is intentionally a near-verbatim port of the
 * modal so the manual e2e flow stays exercisable in the interim.
 */
...
export default function SignInPage() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center px-6 py-16">
      <div className="w-full max-w-[400px] rounded-2xl bg-bg-primary p-8 shadow-xl">
        {step === "email" && (
