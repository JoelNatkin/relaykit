...
- Last updated: 2026-06-06 (Session 127 — marketing-home rebuild on branch `feat/marketing-home` (unmerged): v10 home at `/`, free configurator tool moved to `/messages`, single metallic-gold accent token (D-427/D-428), dark default site-wide. Owed at merge: full file-inventory section for the new `components/home/*` + `configurator-peek` + `/messages` route, the PROTOTYPE_SPEC home/routing rewrite, and PRODUCT_SUMMARY. Prior — Session 126: sole-proprietor customer-path question resolved: deferred at launch as a MASTER_PLAN out-of-scope scope note (no D-number); `no-ein-sole-proprietor-path` exploration promoted/retired from the active list. No new decisions.)
