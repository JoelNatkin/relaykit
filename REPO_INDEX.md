...
| `DECISIONS.md` | 2026-05-20 (Session 100 — D-408 + D-400 supersession annotation) | Active product decisions D-84+. |
...
| `PROTOTYPE_SPEC.md` | 2026-05-20 (Session 100 — flat-message-model collapse, D-408) | Screen-level UI specs for `/prototype` and stabilized marketing-site surfaces. |
...
| `marketing-site/lib/message-library/[category].ts` | 6 remaining per-category typed stubs (appointments, customer-support, marketing, team-alerts, community, waitlist) — empty `variables`, empty `compliance.rules`, empty `messages: []` (flat shape per D-408, ready for later authoring). |
