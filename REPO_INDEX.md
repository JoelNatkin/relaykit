...
- Branch state: `main` only — no unmerged feature branches. Session 100 landed `feat/order-updates-authoring` (Order updates corpus authoring, first `WorkflowCategory`) via `--no-ff` merge (`76b7dfb`), plus the Session 100 close-out commit. Branch deleted local + remote post-merge.
...
| `PROTOTYPE_SPEC.md` | 2026-05-20 (Session 100 — Order updates live note) | Screen-level UI specs for `/prototype` and stabilized marketing-site surfaces. |
...
## message-library (Session 91 scaffold; Session 94 Verification authoring; Session 95 configurator integration; Session 99 Account events authoring; Session 100 Order updates authoring)

Wave 2 message-library — a typed corpus of SMS message templates organized by category. Scaffolded Session 91 on `feat/message-library-scaffold`; Session 94 extended the schema corpus-wide (tone variants, per-category variable catalogs, compliance blocks) and authored the first category, Verification, on `feat/verification-message-authoring` (merged to main). Session 95 (`feat/configurator-one-corpus`) added a category `description` field + a `Sub.tooltip` field, and made the home-page configurator consume the corpus directly (see "configurator" section below). Session 99 (`feat/account-events-authoring`) authored the second category, Account events, and added the `account_link` variable (workspace-settings-sourced, developer's own domain). Session 100 (`feat/order-updates-authoring`) authored the third category and the first `WorkflowCategory` in the corpus, Order updates, adding six SDK-payload variables (`order_number`, `tracking_link`, `estimated_delivery`, `return_link`, `refund_amount`, `card_type`). The other 6 categories remain empty stubs pending later authoring passes.
...
| `marketing-site/lib/message-library/order-updates.ts` | Order updates category — **populated** (Session 100): first `WorkflowCategory` in the corpus. 7 stages (Order confirmation → Processing → Shipping confirmation → Out for delivery → Delivered → Return initiated → Refund processed) / 7 messages / 21 tone variants, 7-variable catalog (workspace_name + 6 SDK-payload tokens), 6-rule compliance block. `tracking_link` accepts well-known carrier-domain URLs (UPS/USPS/FedEx) and discourages public shorteners; `return_link` is the developer's own returns UI on their own domain — RelayKit does not shorten or host. All 21 bodies are ASCII-only; max charCount 137/160 at worst-case substitution. Governs against D-393/D-394/D-398/D-399/D-402. |
| `marketing-site/lib/message-library/[category].ts` | 6 remaining per-category typed stubs (appointments, customer-support, marketing, team-alerts, community, waitlist) — empty subs/stages, empty `variables`/`compliance`. |
