...
- **RelayKit vertical-to-Sinch-use-case mapping for customer campaign registration** — The Sinch 10DLC dashboard exposes ~22 use cases (2FA, Account Notification, Customer Care, Delivery Notification, Marketing, Mixed, Low Volume Mixed, Charity, Higher Education, K-12, Polling and Voting, Public Service Announcement, Security Alert, Social, Sweepstake, Emergency, Fraud Alert, Agents and Franchises, Machine to Machine, Proxy, Direct Lending, plus Carrier Exemptions); RelayKit currently exposes 8 verticals (Appointments, Verification, Orders, Support, Marketing, Internal, Community, Waitlist) which are a curated subset using product-language re-categorization. Per PRODUCT_SUMMARY §14 "Intake interrogates" and §4 "Vertical-to-carrier-use-case mapping is internal," translation must be perfect since customers never see Sinch taxonomy. Phase 5 design needs to: (1) confirm correct mapping for each existing vertical (which Sinch use case does each map to?), (2) decide whether some Sinch use cases warrant new RelayKit verticals (Charity for nonprofits, Higher Ed for university customers — both real customer segments not currently covered), (3) decide whether some Sinch use cases should remain explicitly excluded (Political, Sweepstake, Polling, Fraud Alert — likely out of scope for ICP). Also: when a customer campaign should be LVM vs single-use-case standard vs Mixed — affects throughput tier and pricing for that customer. (Origin: 2026-04-26 chat — surfaced during Experiment 3b campaign creation)
